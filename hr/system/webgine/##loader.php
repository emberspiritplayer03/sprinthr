...
class Loader
{
	...
	{
		define("LIB_PATH", $_SERVER['DOCUMENT_ROOT']. MAIN_FOLDER.'/application/libraries'.LIBRARY_FOLDER);

		$class = ucfirst($class);
		if (file_exists(LIB_PATH . "$class" . EXT))
		{
			require_once LIB_PATH . "$class" . EXT;
			$object = $class;
	...
	public static function appLibrary($class)
	{
		...
		if (is_array($class))
		{
			...
			{
				$class = $c;
				if (!file_exists(LIB_PATH . "$c" . EXT))
				{
					exit("<b>$c" . EXT . "</b> not found in " . LIB_PATH);
				}
				require_once LIB_PATH . "$c" . EXT;
			}
		}
		else
		{
			...
			if (!file_exists(LIB_PATH . "$class" . EXT))
			{
				exit("<b>$class" . EXT . "</b> not found in " . LIB_PATH);
			}
