...
{
    protected $first_day_of_week = 'Sunday'; //or Saturday
...

	{
...

		{
			...
			$str .= "<td class='cal-title' colspan='{$colspan}'>
        ...
        if ($this->first_day_of_week == 'Saturday') {
    		for ($i = 1; $i <= 7; $i++) {
        ...
        if ($this->first_day_of_week == 'Saturday') {
            ...
            $start_day_of_week = $this->getStartDayOfWeek() + 1;
        }
		...
		for ($i = 1; $i <= count($out); $i++) {
			...
			if ($this->isToday($day))
			{
				$str .= $this->getTodayString($this->month, $day, $this->getActiveYear(),'e');
			}
			else
			{
				$str .= $this->getDayString($this->month, $day, $this->getActiveYear(),'e');
			}
	...
	public function groupDisplayRd()
	{
		...
		for ($i = 1; $i <= count($out); $i++) {
			...
			if ($this->isToday($day))
			{
				$str .= $this->getGroupDayString($this->month, $day, $this->getActiveYear(),'d');
			}
			else
			{
				$str .= $this->getGroupDayString($this->month, $day, $this->getActiveYear(),'d');
    ...
    protected function getDayString($month, $day, $year) {
        return "<td class='cal-day'>" . $day . "</td>";
    }

    protected function getTodayString($month, $day, $year) {
        return "<td class='cal-today' id='cal-today'>" . $day . "</td>";
    }
