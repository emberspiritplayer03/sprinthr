...
class Holiday_Controller extends Controller
{
	...
	function _add_holiday() {
        ...
        if (!Tools::hasValue($_POST['year'])) { $error++; }
		...
		if ($error == 0) {		
			...
			if ($id) {
				...
				$holiday      = $_POST['year'] .'-'. $_POST['month'] .'-'. $_POST['day'];	
				$holiday_date = date("Y-m-d",strtotime($holiday));
				$today 		  = date("Y-m-d");

				$cp = new G_Cutoff_Period();
				$data = $cp->getCurrentCutoffPeriod($today);

				/*if( !empty($data) && $holiday_date >= $data['period_start'] && $holiday_date <= $data['period_end'] && $data['is_payroll_generated'] != G_Cutoff_Period::YES){															
					G_Attendance_Helper::updateAttendanceByAllActiveEmployees($holiday_date); //Update attendance if holiday is within current cutoff period
				}*/

				if( !empty($data) && $data['is_lock'] == G_Cutoff_Period::NO ){
					G_Attendance_Helper::updateAttendanceByAllActiveEmployees($holiday_date); //Update attendance if holiday is within current cutoff period
				}
	...
	function _delete_holiday() {
		...
		if ($is_deleted) {
			...
			/*if( $data['is_payroll_generated'] != G_Cutoff_Period::YES ){
				G_Attendance_Helper::updateAttendanceByAllActiveEmployees($holiday_date);
			}*/
	...
	function _edit_holiday() {
        ...
        if (!Tools::hasValue($_POST['year_'])) { $error++; }
		...
		if ($error == 0 && $is_valid_token) {
			...
			if ($holiday) {
				...
				$holiday_date = $_POST['year_'] .'-'. $_POST['month_'] .'-'. $_POST['day_'];
				if (strtotime($holiday_date) <= strtotime(date('Y-m-d'))) {
					G_Attendance_Helper::updateAttendanceByAllActiveEmployees($holiday_date);
				}*/

				$holiday      = $_POST['year_'] .'-'. $_POST['month_'] .'-'. $_POST['day_'];	
				$holiday_date = date("Y-m-d",strtotime($holiday));
				$today 		  = date("Y-m-d");
	...
	function ajax_show_holiday_list() {
        ...
        if (empty($_GET['year'])) {
            $this->var['year'] = $year = Tools::getGmtDate('Y');
        } else {
            $this->var['year'] = $year = (int) $_GET['year'];
        }

        $this->var['holidays'] = G_Holiday_Finder::findByYear($year);
		$this->view->noTemplate();
		$this->view->render('holiday/ajax_holiday_list.php',$this->var);
	}

	function ajax_show_holiday_calendar() {
		...
		$this->view->noTemplate();
		$this->view->render('holiday/ajax_holiday_calendar.php',$this->var);
	}

	function _load_holiday_data() {
		if (empty($_GET['year'])) {
            $this->var['year'] = $year = Tools::getGmtDate('Y');
        } else {
            $this->var['year'] = $year = (int) $_GET['year'];
        ...
        $holidays = G_Holiday_Finder::findByYear($year);
        $return = array();
        if($holidays) {
        	foreach($holidays as $key => $h) {
        		$r['title'] = $h->getTitle();
        		$date = "{$h->getYear()}-{$h->getMonth()}-{$h->getDay()}";
        		$r['start'] = date("Y-m-d",strtotime($date));
        		array_push($return,$r);
        	}
