<?php
class Payslip_Project_Site_Controller extends Controller
{
	...
	function index() {		
		...
		$this->view->render('payslip_project_site/index.php', $this->var);	
	}
    ...
    function manage() {
        ...
        $this->var['action'] = url('payslip_project_site/manage');
		$this->var['payroll']   = 'class="selected"';
        $this->view->setTemplate('template_project_site.php');
        $this->var['page_title'] = 'Payslip';
        ...
        if ($previous_cutoff) {
            ...
            $this->var['previous_cutoff_link'] = url("payslip_project_site/manage?from={$previous_from}&to={$previous_to}&hpid={$previous_id}");
        }
        ...
        if ($next_cutoff) {
            ...
            $this->var['next_cutoff_link'] = url("payslip_project_site/manage?from={$next_from}&to={$next_to}&hpid={$next_id}");
        }
		
        $this->view->render('payslip_project_site/manage.php',$this->var);
    }
	...
	function show_payslip() {		
		...
		if($employee_access == Sprint_Modules::PERMISSION_05) {
			...
			if($e->getIsConfidential() != 1) {
				redirect("payslip_project_site/manage?from={$from}&to={$to}");
			}
		}elseif($employee_access == Sprint_Modules::PERMISSION_07) {
		...
		//$this->var['module_title'] =  ': <b class="mplynm">' .$e->getName(). '</b>' . "<a class='gray_button title_back_button' href='" . url('payslip_project_site/manage?from='. $from .'&to='. $to). "'><i></i>Back to List</a>";"Payslip: ". '<b class="mplynm">' .$e->getName(). '</b>' . "<a class='gray_button title_back_button' href='" . url('payslip_project_site/manage?from='. $from .'&to='. $to). "'><i></i>Back to List</a>";
        ...
        if (!$p) {
            $this->view->render('payslip_project_site/no_payslip.php', $this->var);
            exit;
        }
       ...
       //end loan balance
		...
		$this->var['payroll']   = 'class="selected"';
		$this->view->setTemplate('template_project_site.php');
		$this->view->render('payslip_project_site/show_payslip.php', $this->var);		
	}
	...
	function processed_payroll() {		
		...
		$this->var['payroll']   = 'class="selected"';
		$this->view->setTemplate('template_project_site.php');
		$this->view->render('payslip_project_site/processed_payroll.php', $this->var);	
	}
	...
	function _processed_payroll_list_dt() {	
		...
		$this->view->render('payslip_project_site/_processed_payroll_list_dt.php', $this->var);	
	}
	...
	function _show_move_deduction_form() {
		...
		if($period){
			...
			$this->view->render('payslip_project_site/_show_move_deduction_form.php', $this->var);
		}else{
	...
	function export_payslip() {
		...
		$this->view->render('payslip_project_site/export_payslip.php', $this->var);
	}
	...
	function download_payslip_old() {
        ...
        $this->view->render('payslip_project_site/download_payslip.php', $this->var);		
	}
   ...
   if ($frequency_id == 2) {
	...
	function add_earning() {
		$this->var['action'] = url('payslip_project_site/_add_earning');
		...
		$this->var['payroll']   = 'class="selected"';
		$this->view->setTemplate('template_project_site.php');
		$this->view->render('payslip_project_site/add_earning_form.php', $this->var);
	}
	
	function add_deduction() {
		$this->var['action'] = url('payslip_project_site/_add_deduction');
		$this->var['employee_id'] = $_GET['employee_id'];
		$this->var['from'] = $_GET['from'];
		$this->var['to'] = $_GET['to'];
		$this->var['payroll']   = 'class="selected"';
		...
		$this->view->setTemplate('template_project_site.php');
		$this->view->render('payslip_project_site/add_deduction_form.php', $this->var);
	}
	
	function change_deduction_amount() {
		$this->var['action'] = url('payslip_project_site/_change_deduction_amount');
		$this->var['employee_id'] = $_GET['employee_id'];
		$this->var['from'] = $_GET['from'];
		$this->var['to'] = $_GET['to'];
		...
		$this->var['payroll']   = 'class="selected"';
		$this->view->setTemplate('template_project_site.php');
		$this->view->render('payslip_project_site/change_deduction_amount_form.php', $this->var);
	}
	...
	function dialog_change_period_payout_date() {
		...
		$this->view->noTemplate();
		$this->view->render('payslip_project_site/dialog_change_period_payout_date_form.php', $this->var);
	}
	...
	function payslip_preview()
	{
		...
		if($pName){
			$location = 'http://' . $_SERVER['HTTP_HOST'] . BASE_FOLDER . "files/payslip_project_site/payslip.pdf";	
			header("Location: {$location}");
			//header('Location: http://gleent.local/gleent_hr_payroll/hr/files/payslip_project_site/payslip.pdf');
		}
	}
