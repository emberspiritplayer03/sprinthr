...
if($employee_access == Sprint_Modules::PERMISSION_05) {
	...
	$search = " AND (a.is_confidential = 1) ";	
}elseif($employee_access == Sprint_Modules::PERMISSION_07) {
	$search = " AND (a.is_confidential = 0) ";
}else{
	$search = "";
}

$next_id = G_Employee_Helper::getNextId(Utilities::decrypt($employee_id),$search);
$next_applicant = G_Employee_Finder::findById($next_id);
$next_hash = G_Employee_Helper::getNextHash(Utilities::decrypt($employee_id),$search);
	
$e = G_Employee_Finder::findByIdBothArchiveAndNot($next_id); //findById($next_id);
...
$prev_id = G_Employee_Helper::getPreviousId(Utilities::decrypt($employee_id),$search);
$prev_applicant = G_Employee_Finder::findByIdBothArchiveAndNot($prev_id); //findById($prev_id);
$prev_hash = G_Employee_Helper::getPreviousHash(Utilities::decrypt($employee_id),$search);
...
if($prev_id) {
	$previous_record = '<a title="'.$tipsy_prev.'" class="tooltip_prev" href="'. url('employee/profile?eid='.Utilities::encrypt($prev_id).'&hash='.$prev_hash.'#personal_details') .'"><span>Previous Record</span></a>';	
}
else {
	$previous_record = '<strong class="disabled_prev"><span>Previous Record</span></strong>';	
}
if($next_id) {
	$next_record  = '<a title="'.$tipsy_next.'" class="tooltip_next" href="'. url('employee/profile?eid='.Utilities::encrypt($next_id).'&hash='.$next_hash.'#personal_details') .'"><span>Next Record</span></a>';	
}
else {
