...
?>
  <tr style="height:50px;">
    ...
    <td style="border:1px solid;" width="80">Union Dues/Agency Fee</td>
...
<?php foreach( $employee as $e  ){ ?>  
  <?php 
    ...
    if ($frequency == 2) {
      $payslip_yearly_breakdown = G_Weekly_Payslip_Helper::computeEmployeeYearlyPayslipBreakdownByEndDate($employee_id, $end_date);
    } 
    else {
    ...
    // if($add_13th_month){
    //   // $bonus = $payslip_section_other_earnings['13th_month_bonus']['value'];
    //   // $add_13th_month_to_total_other_earnings = $payslip_section_other_earnings['13th_month_bonus']['value'];
    //   $a_13th_month_bonus = $month_13th;
    // }
    ...
    $exist_deductions   = array('emergency_loan','salary_loan', 'hmo','sss_loan', 'sss_salary_loan', 'sss_calamity_loan', 'adjustment','witholding tax','philhealth','pagibig', 'pagibig_loan', 'pagibig_salary_loan', 'pagibig_calamity_loan', 'sss','absent_amount','undertime_amount','late_amount','company_loan','education_loan','eo','union_dues','agency_fee');
    foreach($payslip_section_deduction as $deduction_key => $e_deduction) {
      ...
      if (in_array($deduction_key, $exist_deductions)) {
        $is_exists = true;
      }    
  ...
  foreach( $leave_conversion as $key ){
    ...
    if($add_bonus_to_earnings) {
      $other_bonuses += $payslip_section_other_earnings['service_award']['value'];
      $net_pay -= $payslip_section_yearly_bonus['yearly_bonus']['value']['amount'];
    }
    //  if($add_13th_month){
    //   // $bonus = $payslip_section_other_earnings['13th_month_bonus']['value'];
    //   // $add_13th_month_to_total_other_earnings = $payslip_section_other_earnings['13th_month_bonus']['value'];
    //   $other_bonuses += $month_13th;
    // }
    if($add_13th_month){
      $other_bonuses += $payslip_section_yearly_bonus['yearly_bonus']['value']['amount'];

      $net_pay += $other_bonuses;
      $deductnp = $payslip_section_other_earnings['bonus']['value'] + $payslip_section_other_earnings['service_award']['value'];
      $net_pay -= $deductnp;
      
    }

    $earnings_ot_ctpa      = array('ot_allowance','ctpa/sea','ctpa','ctpa_sea','13th_month','non_taxable_converted_leave','taxable_converted_leave','bonus','service_award');  
  ...
  $add_bonus_from_earnings = false;
  $bonus_from_earnings = 0;
  if( !empty($payslip_section_other_earnings['bonus']['value']) && $payslip_section_other_earnings['bonus']['value'] > 0 ) {
    if($add_bonus_to_earnings) {
      //$bonus_from_earnings += $payslip_section_other_earnings['bonus']['value'];

        if($add_13th_month){
          $net_pay += $payslip_section_other_earnings['bonus']['value'];
        }
          
        $other_bonuses += $payslip_section_other_earnings['bonus']['value'];

      //$other_bonuses += $bonus_from_earnings;
  ...
  if($other_earnings > $to_deduct_earnings) {
    $other_earnings = $other_earnings - $to_deduct_earnings;
  }
  
  $total_other_earnings = $payslip_section_other_earnings['ot_allowance']['value'] + $meal_transpo_earnings + $other_earnings + $rice_earnings + $total_ctpa_sea + $bonus_from_earnings + $bonus_leave_converted + $other_bonuses;
...
<?php
  $net_pay = 0;
  if($show_13th_month_only) {
    $thirteenth_month_bonus_leavecon    = ($payslip_section_yearly_bonus['yearly_bonus']['value']['amount'] + $payslip_section_other_earnings['bonus']['value']);  // + $bonus_leave_converted
    $service_award                      = $payslip_section_other_earnings['service_award']['value'];

    $bonus_service_award_witholding_tax = $payslip_section_deduction['bonus_/_service_award_witholding_tax']['value'];
    $net_pay += ($thirteenth_month_bonus_leavecon + $service_award) - $bonus_service_award_witholding_tax;
    
  } else {

    $thirteenth_month_bonus_leavecon = 0;
    $bonus_service_award_witholding_tax = 0;
    $service_award = 0;
    $net_pay = 0;
  }
 ...
 ?>
  ...
  <?php 
    ...
    if($show_13th_month_only){
       $sub_total['total_basic_pay']      += 0;
    $sub_total['total_nightshift_diff'] += 0;
    $sub_total['total_overtime_amount'] += 0;
    $sub_total['total_absent_amount']   += 0;
    $sub_total['total_undertime_amount'] += 0;
    $sub_total['total_late_amount']      += 0;
    $sub_total['total_earnings_benefits_adjustment'] += 0;
    $sub_total['total_holiday_pay'] += 0;
    $sub_total['total_position_allowance'] += 0;
    $sub_total['total_gross_pay'] += 0;
    $sub_total['total_witholding'] += 0;
    $sub_total['total_philhealth'] += 0;
    $sub_total['total_sss'] += 0;
    $sub_total['total_pagibig'] += 0;
    $sub_total['total_sss_loan'] += 0;
    $sub_total['total_pagibig_loan'] += 0;
    $sub_total['total_company_emergency_loan'] += 0;
    $sub_total['total_education_loan'] += 0;
    $sub_total['total_hmo_loan'] += 0;
    $sub_total['total_eo_loan'] += 0;
    $sub_total['total_union_dues_agency_fee'] += 0;
    $sub_total['total_other_deductions'] += 0;
    $sub_total['total_deductions'] += 0;
    $sub_total['total_ot_allowance'] += 0;
    $sub_total['total_meal_transpo_allowance'] += 0;
    $sub_total['total_rice_allowance'] += 0;
    $sub_total['total_other_bonuses']  += $thirteenth_month_bonus_leavecon;
    $sub_total['total_other_earnings'] += 0;
    $sub_total['total_of_other_earnings'] += 0;
  ...
  <?php if($show_13th_month_only) { ?>
            ...
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            ...
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format($bonus_service_award_witholding_tax,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            ...
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format($thirteenth_month_bonus_leavecon,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format($service_award,2, '.' , ',' ); ?></td>
            <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format(0,2, '.' , ',' ); ?></td>
  ...
  <?php } else { ?>
    ...
    <td style="font-size:11pt;mso-number-format:'\@';text-align:right;border:1px solid;" align="right"><?php echo number_format($payslip_section_deduction['union_dues']['value'] + $payslip_section_deduction['agency_fee']['value'],2, '.' , ','); ?></td>
...
<?php } ?>
   ...
   <tr style="height:50px;border:1px solid;">
    ...
    <td width="80" style="border:1px solid;">Union Dues/Agency Fee</td>
  ...
  <tr style="height:30px;">
    ...
    <td width="80" style="font-size:14pt;font-weight:bold;border:1px solid;">Union Dues/Agency Fee</td>
