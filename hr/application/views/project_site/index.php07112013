...
<div style="float:right"><a class="gray_button" href="<?php echo url('attendance/face_recognition_timesheet_generator');?>"><i class="icon-file"></i> Attendance File Generator</a><a class="gray_button" href="<?php echo url('attendance/attendance_logs');?>"><i class="icon-align-justify"></i> View Attendance Logs</a></div>
...
?>
  <tr>
    <td class="payslip_period"><strong><a href="<?php echo url('attendance/manage?from='. $period['start'] .'&to='. $period['end'] . '&hpid=' . Utilities::encrypt($period['id']));?>"><?php echo Tools::getGmtDate('M j', strtotime($period['start']));?> - <?php echo Tools::getGmtDate('M j', strtotime($period['end']));?></a></strong></td>
    ...
    <a class="dropbutton" href="<?php echo url('attendance/manage?from='. $period['start'] .'&to='. $period['end'] . '&hpid=' . Utilities::encrypt($period['id']));?>"><i class="icon-zoom-in icon-fade"></i> View List</a>
    </div>
   <!-- <div id="dropholder" style="width:50%">
    <a class="dropbutton" href="<?php echo url('attendance/filter_by_range?from='. $period['start'] .'&to='. $period['end'] . '&hpid=' . Utilities::encrypt($period['id']));?>"><i class="icon-zoom-in icon-fade"></i> Filter by Range</a>
    </div>-->
    </td>
    <td class="vertical-middle">
    	...
    	<a class="btn btn-mini" href="<?php echo url('attendance/download_timesheet_breakdown?from='. $from .'&to='. $to);?>">Breakdown</a>
