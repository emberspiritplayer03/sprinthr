...
<script>
    $(function() {
        ...
        load_no_sched_dt('<?php echo $from; ?>', '<?php echo $to; ?>');
        //$("table").tablesorter();
        ...
        var emp_selected = new $.TextboxList('#emp_selected_no_sched', {
            unique: true,
            plugins: {
                autocomplete: {
                    minLength: 1,
    ...
    function checkUncheck() {
        if ($('input[name="dtrChkNoSched[]"]:checked').length > 0) {
            $('#chkActionNoSched').attr('disabled', false);
            if ($('input[name="dtrChkNoSched[]"]:checked').length == $('input[name="dtrChkNoSched[]"]').length) {
                $('#chkAllNoSched').attr('checked', true);
            }
        } else {
            $('#chkActionNoSched').attr('disabled', true);
        }
    }

    function chkAllNoSched() {
        if ($('#chkAllNoSched:checked').length) {
            $('#chkActionNoSched').attr('disabled', false);
            $('input[name="dtrChkNoSched[]"]').attr('checked', true);
        } else {
            $('#chkActionNoSched').attr('disabled', true);
            $('input[name="dtrChkNoSched[]"]').attr('checked', false);
        }
    }
</script>
...
<div id="">
    <div id="employee_search_container" style="overflow:visible !important; padding-bottom:15px;">
        <div class="employee_basic_search searchcnt" id="search_wrapper">
            ...
            <div class="float-left">
                ...
                <div id="all_emp_no_sched" class="float-left" style="width:312px;">
                    <input disabled="disabled" type="text" name="input_disabled" id="input_disabled" style="width:290px; min-width:290px;" value="" />
                </div>
                <div id="autocomplete_no_sched" class="float-left">
                    <input type="text" name="emp_selected" id="emp_selected_no_sched" />
                </div>
                <div class="clear"></div>
                <span class="float-left" style=" width:50px;">&nbsp;</span>
                <div class="float-left"><label><input checked="checked" type="checkbox" class="chk_employee" id="chk_employee" name="chk_employee" onclick="javascript:chkEmployeeNoSched(this);" />All employees</label></div>
                <div class="clear"></div>
            </div>
    ...
    <div class="group-2">
        <div class="dt_top_nav">
            <select name="chkActionNoSched" id="chkActionNoSched" onchange="javascript:withSelectedLogsNoSched(this.value);" disabled="disabled">
