...
<script>
    $(function() {
        ...
        load_multiple_out_dt('<?php echo $from; ?>', '<?php echo $to; ?>');
        //$("table").tablesorter();
        ...
        var emp_selected = new $.TextboxList('#emp_selected_multiple_out', {
            unique: true,
            plugins: {
                autocomplete: {
                    minLength: 1,
    ...
    function checkUncheck() {
        if ($('input[name="dtrChkMultipleOut[]"]:checked').length > 0) {
            $('#chkActionMultipleOut').attr('disabled', false);
            if ($('input[name="dtrChkMultipleOut[]"]:checked').length == $('input[name="dtrChkMultipleOut[]"]').length) {
                $('#chkAllMultipleOut').attr('checked', true);
            }
        } else {
            $('#chkActionMultipleOut').attr('disabled', true);
        }
    }

    function chkAllMultipleOut() {
        if ($('#chkAllMultipleOut:checked').length) {
            $('#chkActionMultipleOut').attr('disabled', false);
            $('input[name="dtrChkMultipleOut[]"]').attr('checked', true);
        } else {
            $('#chkActionMultipleOut').attr('disabled', true);
            $('input[name="dtrChkMultipleOut[]"]').attr('checked', false);
        }
    }
</script>
...
<div id="">
    <div id="employee_search_container" style="overflow:visible !important; padding-bottom:15px;">
        <div class="employee_basic_search searchcnt" id="search_wrapper">
            ...
            <div class="float-left">
                ...
                <div id="all_emp_multiple_out" class="float-left" style="width:312px;">
                    <input disabled="disabled" type="text" name="input_disabled" id="input_disabled" style="width:290px; min-width:290px;" value="" />
                </div>
                <div id="autocomplete_multiple_out" class="float-left">
                    <input type="text" name="emp_selected" id="emp_selected_multiple_out" />
                </div>
                <div class="clear"></div>
                <span class="float-left" style=" width:50px;">&nbsp;</span>
                <div class="float-left"><label><input checked="checked" type="checkbox" class="chk_employee" id="chk_employee" name="chk_employee" onclick="javascript:chkEmployeeMultipleOut(this);" />All employees</label></div>
                <div class="clear"></div>
            </div>
    ...
    <div class="group-2">
        <div class="dt_top_nav">
            <select name="chkActionMultipleOut" id="chkActionMultipleOut" onchange="javascript:withSelectedLogsMultipleOut(this.value);" disabled="disabled">
