    ...
    <div class="earnings_period_selected">
        ...
        <?php if (strtotime($start_date) && strtotime($end_date)):?>
            <h2 class="no-padding no-margin">
              ...
              <form method="get" action="<?php echo url('project_site/manage'); ?>">  
    ...
    <tbody>
      <?php foreach ($employees as $e):?>
        <tr>
          ...
          <td><a href="<?php echo url('project_site/show_attendance?employee_id='. Utilities::encrypt($e->getId()) .'&hash='. $e->getHash() .'&from='. $start_date .'&to='. $end_date.'&selected_frequency='.$frequency_id);?>"><?php echo $e->getLastname();?>  <?php echo $e->extension_name;?> , <?php echo $e->getFirstname();?> <?php echo $e->middlename[0]; ?>. </a></td>
          <td>
            <?php if ($employee_break_logs_summary_helper->checkHasIncompleteBreakByDateRangeEmployeeId($start_date, $end_date, $e->getId())) {?>
              <div class="exclamation-triangle-icon"></div>
