    ...
    <div class="earnings_period_selected">
        <div class="overtime_title_period"><?php echo $period_selected; ?>
            ...
            <div class="pull-right" style="position:relative; top:-3px; font-size: 12px !important; margin-right: 8px !important;">
              <form method="get" action="<?php echo url('ob/approved'); ?>">  
...
<?php if($is_period_lock == ((!isset($frequency_id) || $frequency_id != 2 ? G_Cutoff_Period::NO : G_Weekly_Cutoff_Period::NO))){ ?>   
    <div class="datatable_withselect display-inline-block right-space">
        <select disabled="disabled" name="chkAction" id="chkAction" onchange="javascript:withSelectedApproved(this.value);">
            ...
            <option value="ob_disapprove">Disapprove</option>      
            <!-- <option value="ob_archive">Send to Archive</option>      -->                
        </select>
    </div>    
<?php }else{ ?>
...
<script>
	$(function() {
         load_approved_ob_list_dt("<?php echo $period['from']; ?>","<?php echo $period['to']; ?>","<?php echo $selected_frequency; ?>");       
	});
</script>