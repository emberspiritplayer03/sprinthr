...
<script>
    function changePayPeriodByYear(selected_year,selected_cutoff,class_container, selected_frequency = 1) {
        ...
        $.get(base_url + 'ob/ajax_load_payroll_period_by_year',{selected_year:selected_year,selected_cutoff:selected_cutoff, selected_frequency:selected_frequency},
          function(o){
    ...
    <div class="earnings_period_selected">
        <div class="overtime_title_period"><?php echo $period_selected; ?>
	        <div class="pull-right" style="position:relative; top:-3px; font-size: 12px !important; margin-right: 8px !important;">
	          <form method="get" action="<?php echo url('ob/archives'); ?>">  
...
<?php if($is_period_lock == ((!isset($frequency_id) || $frequency_id != 2 ? G_Cutoff_Period::NO : G_Weekly_Cutoff_Period::NO))){ ?>   
    <div class="datatable_withselect display-inline-block right-space">
        <select disabled="disabled" name="chkAction" id="chkAction" onchange="javascript:withSelectedArchived(this.value);">
            <option value="">With Selected:</option>            
            <option value="ob_restore">Restore</option>                                       
        </select>
    </div>    
<?php }else{ ?>
...
<?php } ?>
	<?php if($download_url){ ?>
        <a id="import_undertime" class="gray_button" href="<?php echo $download_url; ?>"><i class="icon-excel icon-custom vertical-middle"></i> <b>Download OB</b></a>
    <?php } ?>
    <div class="clear"></div>
</div>
    <div id="ob_list_dt_wrapper" class="dtContainer"></div>    
</form>
<script>
	$(function() {
   load_archived_ob_list_dt("<?php echo $period['from']; ?>","<?php echo $period['to']; ?>","<?php echo $selected_frequency; ?>");       
	});
</script>
