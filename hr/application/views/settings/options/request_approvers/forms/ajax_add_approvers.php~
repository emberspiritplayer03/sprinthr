...
$(function() {		
	var emp_pos = new $.TextboxList('#emp_pos', {unique: true,plugins: {
			autocomplete: {
				...
				onlyFromValues: true,
				queryRemote: true,
				remote: {url: base_url + 'settings/ajax_get_positions_employees_autocomplete'}
			
			}
		}});
});
$(function(){
	$('ul.textboxlist-bits').attr("title","Type position or employee name to see suggestions.");
          ...
          <tr>
            <td>
             ...
             <div id="txt_positions">
               <input class="validate[required] text-input text" type="text" name="emp_pos" id="emp_pos" />
             </div>                             
            </td>
          </tr>
          <!--<tr>
