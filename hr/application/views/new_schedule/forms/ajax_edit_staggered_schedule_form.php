			...
			<table class="no_border" width="100%">
				...
				<?php if (!$is_default){ ?>
					<tr>
						...
						<td><input onchange="onValueChanged()" class="validate[required] text-input date_input" type="text" name="start_date" id="start_date" value="<?php echo $effectivity_date; ?>" /></td>
					</tr>
					<tr>
						<td class="field_label">*End Date:</td>
						<td><input onchange="onValueChanged()" class="validate[required] text-input date_input" type="text" name="end_date" id="end_date" value="<?php echo $end_date; ?>" /></td>
					</tr>
				<?php }
					foreach ($days as $number => $day) { 
					$counter_for_work_days_array = 0;
					$counter_for_hours_array = 0;
					$short_day = strtolower(substr($day, 0, 3));
					foreach ($working_days as $work_day) {
						if ($short_day == $work_day) {
							break;
						}
						$counter_for_work_days_array++;
					}
					$day_hour = '';
					foreach($hours as $hour){
						if($counter_for_hours_array == $counter_for_work_days_array){
							$day_hour = $hour;
							break;
						}
						$counter_for_hours_array++;
					}
					?>
					<tr>
						<td class="field_label"><?php echo $day; ?></td>
						<td>
							<input class="time_in" name="day_hours[<?php echo $short_day; ?>]" value="<?php echo $day_hour; ?>" type="text" id="start_time_<?php echo $day_hour; ?>" style="width:60px" /> Hours
							<!--<span <?php echo $clear_link_visible; ?> id="clear_link_<?php echo $hour; ?>" class="clear_link"><a class="btn btn-mini" href="javascript:clearTime(<?php echo $hour; ?>)"><i class="icon-remove-circle"></i> Clear</a></span>
...
<script>
	...
	$("#edit_schedule_form #start_date").datepicker({
		dateFormat: 'yy-mm-dd',
		changeMonth: true,
		changeYear: true,
		showOtherMonths: true,
		onSelect: function(date) {
			$("#edit_schedule_form #end_date").datepicker('option', {
				minDate: $(this).datepicker('getDate')
			});
		}
	});

	$("#edit_schedule_form #end_date").datepicker({
		dateFormat: 'yy-mm-dd',
		changeMonth: true,
		changeYear: true,
		showOtherMonths: true
