...
<input type="hidden" name="geid" value="<?php echo $company_data['id']; ?>" />
<input type="hidden" name="token" id="token" value="<?php echo $token; ?>" />
<div id="form_main" class="inner_form popup_form wider">
    <div id="form_default">
    <h3>Department / Section : <?php echo $company_data['title']; ?><br /><small style="font-size:11px" class="red">Note : Day(s) with empty fields will be mark as restday</small></h3>   
    <table class="no_border" width="100%">
    	<tr>
        <td width="24%" class="field_label">*Schedule Name:</td>
            <td width="76%"><input class="validate[required] text-input" type="text" name="schedule[name]" id="name" value="" /></td>
        </tr>      
        <tr>
          <td class="field_label">*Start Date:</td>
          <td><input class="validate[required] text-input date_input" type="text" name="schedule[start_date]" id="start_date" value="" /></td>
        </tr> 
        <tr>
          <td class="field_label">*End Date:</td>
          <td><input class="validate[required] text-input date_input" type="text" name="schedule[end_date]" id="end_date" value="" /></td>
        </tr>        
        ...
        <tr>
          ...
          <td>
          	<input class="time_in" name="schedule[time_in][<?php echo $short_day;?>]" value="<?php echo $time_in;?>" onchange="onStartTimeChanged(<?php echo $number;?>)" type="text" id="start_time_<?php echo $number;?>" style="width:60px"/> to 
          	<input name="schedule[time_out][<?php echo $short_day;?>]" value="<?php echo $time_out;?>" onchange="onEndTimeChanged(<?php echo $number;?>)" type="text" id="end_time_<?php echo $number;?>" style="width:60px" />
            <?php 
			$clear_link_visible = '';
			if ($time_in == '' || $time_out == '') {
...
$("#add_schedule_form #start_date").datepicker({
	...
	onSelect: function(date){
       $("#add_schedule_form #end_date").datepicker('option',{minDate:$(this).datepicker('getDate')});
    }
});
