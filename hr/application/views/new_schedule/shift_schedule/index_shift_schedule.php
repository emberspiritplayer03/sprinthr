...
<script type="text/javascript">
	$(function() {
        ...
        $("#tabs").tabs({});
		load_shift_schedule("#schedule");


		$(".no_schedule").click(function(){
			load_no_shift_schedule('#no_schedule');
		});
        $(".leave").click(function(){
			load_leave_shift_schedule('#leave');
		});
        $(".rest_day").click(function(){
			load_rest_day_shift_schedule('#rest_day');
		});
        $(".ob").click(function(){
            load_ob_shift_schedule('#official_business');
		});
        $(".rules").click(function(){

		});
        $(".employee").click(function(){
            load_shift_schedule_employee_list_dt();
		});
        $(".schedule").click(function(){
			load_shift_schedule_list_dt();
		});
            ...
            <div id="form_main" class="inner_form popup_form wider">
                <div id="form_default">
                    ...
                    <table class="no_border" width="100%">
                        <tr>
                            <td class="field_label">AM Shift </td>
                        </tr>
                        <tr>
                            <td class="field_label">*Time In:</td>
                            <td>
                                <input class="validate[required] time_in" name="time_in_am" value="<?php echo $time_in; ?>" onchange="onStartTimeChanged(<?php echo 1; ?>)" type="text" id="start_time_<?php echo 1; ?>" style="width:60px" />
                            </td>
                        </tr>
                        <tr>
                            <td class="field_label">*Time Out:</td>
                            <td>
                                <input class="validate[required]" name="time_out_am" value="<?php echo $time_out; ?>" onchange="onEndTimeChanged(<?php echo 1; ?>)" type="text" id="end_time_<?php echo 1; ?>" style="width:60px" />
                            </td>
                        </tr>
                    </table>
                    <table class="no_border" width="100%">
                        <tr>
                            <td class="field_label">PM Shift </td>
                        </tr>
                        <tr>
                            <td class="field_label">*Time In:</td>
                            <td>
                                <input class="validate[required] time_in" name="time_in_pm" value="<?php echo $time_in_pm; ?>" onchange="onStartTimeChangedPM(<?php echo 1; ?>)" type="text" id="pm_start_time_<?php echo 1; ?>" style="width:60px" />
                            </td>
                        </tr>
                        <tr>
                            <td class="field_label">*Time Out:</td>
                            <td>
                                <input class="validate[required]" name="time_out_pm" value="<?php echo $time_out_pm; ?>" onchange="onEndTimeChangedPM(<?php echo 1; ?>)" type="text" id="pm_end_time_<?php echo 1; ?>" style="width:60px" />
                            </td>
                        </tr>
                    </table>
                </div>
...
<script>
    ...
    function onEndTimeChangedPM(number) {
        if (number == 1) {
            var end_time = $('#pm_end_time_' + number).val();
            if (end_time != '') {
                $('#copy_time_1').show();
            }
        }
    ...
    function onStartTimeChangedPM(number) {
        var start_time_id = '#pm_start_time_' + number;
        var end_time_id = '#pm_end_time_' + number;
        var start_time = $('#pm_start_time_' + number).val();
        var split_time = start_time.split(':');
        var hour = parseFloat(split_time[0]) + 9;
        var split_minutes = split_time[1].split(' ');
        var minutes = split_minutes[0];
    ...
    for (i = 1; i <= 7; i++) {
        $('#pm_start_time_' + i).timepicker({
            'minTime': '6:00 am',
            'maxTime': '5:30 am',
            'timeFormat': 'g:i a'
        });
        $('#pm_end_time_' + i).timepicker({
            'minTime': '6:00 am',
            'maxTime': '5:30 am',
            'timeFormat': 'g:i a'
        });
