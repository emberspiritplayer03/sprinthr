...
<script>
    $(function() {
        ...
        load_no_staggered_schedule_dt();
        //$("table").tablesorter();

        var emp_selected = new $.TextboxList('#emp_selected_no_staggered', {
            unique: true,
            plugins: {
                autocomplete: {
                    minLength: 1,
    ...
    function checkUncheckNoSchedule() {
        if ($('input[name="dtrChkNoSchedule[]"]:checked').length > 0) {
            $('#chkActionNoSchedule').attr('disabled', false);
            if ($('input[name="dtrChkNoSchedule[]"]:checked').length == $('input[name="dtrChkNoSchedule[]"]').length) {
                $('#chkAllNoSchedule').attr('checked', true);
            }
        } else {
            $('#chkActionNoSchedule').attr('disabled', true);
        }
    }

    function chkAllNoSchedule() {
        if ($('#chkAllNoSchedule:checked').length) {
            $('#chkActionNoSchedule').attr('disabled', false);
            $('input[name="dtrChkNoSchedule[]"]').attr('checked', true);
        } else {
            $('#chkActionNoSchedule').attr('disabled', true);
            $('input[name="dtrChkNoSchedule[]"]').attr('checked', false);
        }
    }
</script>
<div id="employee_search_container" style="overflow:visible !important; padding-bottom:15px;">
    <div class="employee_basic_search searchcnt" id="search_wrapper">
        ...
        <div class="float-left">
            ...
            <div id="all_emp_no_staggered" class="float-left" style="width:312px;">
                <input disabled="disabled" type="text" name="input_disabled" id="input_disabled" style="width:290px; min-width:290px;" value="" />
            </div>
            <div id="autocomplete_no_staggered" class="float-left">
                <input type="text" name="emp_selected" id="emp_selected_no_staggered" />
            </div>
            <div class="clear"></div>
            <span class="float-left" style=" width:50px;">&nbsp;</span>
            <div class="float-left"><label><input checked="checked" type="checkbox" class="chk_employee" id="chk_employee_no_staggered" name="chk_employee" onclick="javascript:chkEmployeeNoStaggeredSchedule(this);" />All employees</label></div>
            <div class="clear"></div>
        </div>
        <button class="blue_button" onclick="javascript:load_no_staggered_schedule_dt();"><i class="icon-search icon-white"></i> Search</button>
...
<div>
    <div class="dt_top_nav">
        ...
        <div class="dt_limit">Limit :
            <select id="dt_limit" style="width:50px;" onchange="javascript:load_no_staggered_schedule_dt();">
    ...
    <div class="group-2">
        <div class="dt_top_nav">
            <select name="chkActionNoSchedule" id="chkActionNoSchedule" onchange="javascript:withSelectedLogsNoSchedule(this.value);" disabled="disabled">
    ...
    <table id="" class="formtable">
        <thead>
            <tr>
                <th valign="top" width="2%">
                    <input type="checkbox" id="chkAllNoSchedule" name="chkAllNoSchedule" onchange="chkAllNoSchedule();" original-title="Check All">
                </th>
                ...
                <th valign="top" onclick="javascript:sortDt('type');" width="10%"><strong>Department</strong></th>
        ...
        <tbody id="no_staggered_schedule_dt_wrapper">
        </tbody>
    </table>
    <div class="paginator yui-skin-sam"></div>
</div>
