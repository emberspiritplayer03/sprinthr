...
<script>
    $(function() {
        ...
        load_rest_day_staggered_schedule_dt();
        //$("table").tablesorter();

        var emp_selected = new $.TextboxList('#emp_selected_rd_staggered', {
            unique: true,
            plugins: {
                autocomplete: {
                    minLength: 1,
    ...
    function checkUncheckRestDay() {
        if ($('input[name="dtrChkRestDay[]"]:checked').length > 0) {
            $('#chkActionRestDay').attr('disabled', false);
            if ($('input[name="dtrChkRestDay[]"]:checked').length == $('input[name="dtrChkRestDay[]"]').length) {
                $('#chkAllRestDay').attr('checked', true);
            }
        } else {
            $('#chkActionRestDay').attr('disabled', true);
        }
    }

    function chkAllRestDay() {
        if ($('#chkAllRestDay:checked').length) {
            $('#chkActionRestDay').attr('disabled', false);
            $('input[name="dtrChkRestDay[]"]').attr('checked', true);
        } else {
            $('#chkActionRestDay').attr('disabled', true);
            $('input[name="dtrChkRestDay[]"]').attr('checked', false);
        }
    }
</script>
<div id="employee_search_container" style="overflow:visible !important; padding-bottom:15px;">
    <div class="employee_basic_search searchcnt" id="search_wrapper">
        ...
        <div class="float-left">
            ...
            <div id="all_emp_rd_staggered" class="float-left" style="width:312px;">
                <input disabled="disabled" type="text" name="input_disabled" id="input_disabled" style="width:290px; min-width:290px;" value="" />
            </div>
            <div id="autocomplete_rd_staggered" class="float-left">
                <input type="text" name="emp_selected" id="emp_selected_rd_staggered" />
            </div>
            <div class="clear"></div>
            <span class="float-left" style=" width:50px;">&nbsp;</span>
            <div class="float-left"><label><input checked="checked" type="checkbox" class="chk_employee" id="chk_employee_rd_staggered" name="chk_employee" onclick="javascript:chkEmployeeRestDayStaggeredSchedule(this);" />All employees</label></div>
            <div class="clear"></div>
        </div>
        <button class="blue_button" onclick="javascript:load_rest_day_staggered_schedule_dt();"><i class="icon-search icon-white"></i> Search</button>
...
<div>
    <div class="dt_top_nav">
        ...
        <div class="dt_limit">Limit :
            <select id="dt_limit" style="width:50px;" onchange="javascript:load_rest_day_staggered_schedule_dt();">
    ...
    <div class="group-2">
        <div class="dt_top_nav">
            <select name="chkActionRestDay" id="chkActionRestDay" onchange="javascript:withSelectedLogsRestDay(this.value);" disabled="disabled">
    ...
    <table id="" class="formtable">
        <thead>
            <tr>
                <th valign="top" width="2%">
                    <input type="checkbox" id="chkAllRestDay" name="chkAllRestDay" onchange="chkAllRestDay();" original-title="Check All">
                </th>
        ...
        <tbody id="rest_day_staggered_schedule_dt_wrapper">
        </tbody>
    </table>
    <div class="paginator yui-skin-sam"></div>
</div>
