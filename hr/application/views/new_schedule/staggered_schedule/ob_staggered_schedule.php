...
<script>
    ...
    function checkUncheckOB() {
        if ($('input[name="dtrChkOB[]"]:checked').length > 0) {
            $('#chkActionOB').attr('disabled', false);
            if ($('input[name="dtrChkOB[]"]:checked').length == $('input[name="dtrChkOB[]"]').length) {
                $('#chkAllOB').attr('checked', true);
            }
        } else {
    ...
    function chkAllOB() {
        if ($('#chkAllOB:checked').length) {
            $('#chkActionOB').attr('disabled', false);
            $('input[name="dtrChkOB[]"]').attr('checked', true);
        } else {
            $('#chkActionOB').attr('disabled', true);
            $('input[name="dtrChkOB[]"]').attr('checked', false);
        }
    }
</script>
<div id="employee_search_container" style="overflow:visible !important; padding-bottom:15px;">
    <div class="employee_basic_search searchcnt" id="search_wrapper">
        ...
        <div class="float-left">
            ...
            <div id="all_emp_ob_staggered" class="float-left" style="width:312px;">
                <input disabled="disabled" type="text" name="input_disabled" id="input_disabled" style="width:290px; min-width:290px;" value="" />
            </div>
            <div id="autocomplete_ob_staggered" class="float-left">
                <input type="text" name="emp_selected" id="emp_selected_ob_staggered" />
            </div>
            <div class="clear"></div>
            <span class="float-left" style=" width:50px;">&nbsp;</span>
            <div class="float-left"><label><input checked="checked" type="checkbox" class="chk_employee_ob_staggered" id="chk_employee" name="chk_employee" onclick="javascript:chkEmployeeOfficialBusinessStaggeredSchedule(this);" />All employees</label></div>
            <div class="clear"></div>
        </div>
        <button class="blue_button" onclick="javascript:load_ob_staggered_schedule_dt();"><i class="icon-search icon-white"></i> Search</button>
...
<div>
    <div class="dt_top_nav">
        ...
        <div class="dt_limit">Limit :
            <select id="dt_limit" style="width:50px;" onchange="javascript:load_ob_staggered_schedule_dt();">
    ...
    <div class="group-2">
        <div class="dt_top_nav">
            <select name="chkActionOB" id="chkActionOB" onchange="javascript:withSelectedLogsOB(this.value);" disabled="disabled">
    ...
    <table id="" class="formtable">
        <thead>
            <tr>
                <th valign="top" width="2%">
                    <input type="checkbox" id="chkAllOB" name="chkAllOB" onchange="chkAllOB();" original-title="Check All">
                </th>
        ...
        <tbody id="ob_staggered_schedule_dt_wrapper">
        </tbody>
    </table>
    <div class="paginator yui-skin-sam"></div>
</div>
