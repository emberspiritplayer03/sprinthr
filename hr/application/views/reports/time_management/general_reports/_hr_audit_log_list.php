...
<script> 
	$(function() {		
		  var oTable = $('#audit_trail_hr_dt').dataTable( {
	       "aoColumns": [              
...
<div class="at_filter">
          ...
          <div id="download_hr_link" style="display:block;">
           <a id="download_hr" class="gray_button vertical-middle" href="<?php echo $at_hr_download_url.'?search_col=&search_field='; ?>"><i class="icon-excel icon-custom vertical-middle"></i> <b>Download HR List</b></a>
          </div>
          <div id="download_hr_filter" style="display:block;"></div>

          <div id="at_filter_dropdown">

                Search By:
                <select id="audit_trail_hr_filter">
               ...
               <input type="button" name="Search" id="btn_search" class="gray_button vertical-middle" value="Search">
          </div>
        
</div>

    <div id="tbl_audit_trail_hr_dt" style="display:block;">

        <table id="audit_trail_hr_dt" class="display" style="width:100%; font-size:12px;">
    ...
    <div id="tbl_filter_audit_trail_hr_dt" style="display:none;"></div>
...
<script>
 $("document").ready(function () {
     ...
     $("#tbl_audit_trail_hr_dt").show();

      $("#btn_search").click(function () {

          var SearchCol = $('#audit_trail_hr_filter').val();
          var SearchField = $('#search_value').val();

          $('#download_hr_link').hide();
          //console.log(SearchCol+' = '+SearchField);
          var download_link = '<a id="download_hr" class="gray_button vertical-middle" href="<?php echo $at_hr_download_url;?>?search_col='+SearchCol+'&search_field='+SearchField+'"><i class="icon-excel icon-custom vertical-middle"></i> <b>Download HR List</b></a>';

          $('#download_hr_filter').html(download_link);
          
           $.ajax({
              url: base_url + 'reports/filter_load_hr_audit_log_list',
              type: 'GET',
              ...
              success:function(data){         
                ...
                $("#tbl_audit_trail_hr_dt").hide();

                $("#tbl_filter_audit_trail_hr_dt").show();
                $("#tbl_filter_audit_trail_hr_dt").html(data);
                 //console.log(data);
              }
          });
