...
<table border="1" cellpadding="2" cellspacing="1" style="font-size:9pt; width:836pt; line-height:12pt;">
	<tr>
        ...
        <?php foreach( $leave_type as $lt ){ ?>
            <?php if($lt->getId() == 10 || $lt->getId() == 11) { ?>
	...
	?>
    	<tr>
            ...
            <td align="left" valign="top" style="border-bottom:none;">
                <?php 
                    if( isset($leave_credits[$key]) && $leave_credits[$key] > 0){
                        echo $leave_credits[$key];
                    }else{
                        echo 0;
                    } 
                ?>
            ...
            <?php foreach( $leave_type as $lt ){ ?>
                <?php if($lt->getId() == 10 || $lt->getId() == 11) { ?>
                        <td align="left" valign="top" style="border-bottom:none;">              
                            <?php 
                                if( isset($l['leave_details'][$lt->getId()]) && $l['leave_details'][$lt->getId()]['no_of_days_alloted'] > 0){
                                    echo $l['leave_details'][$lt->getId()]['no_of_days_alloted'];
                                    //$total_leave_used += $l['leave_details'][$lt->getId()]['no_of_days_used'];
                                    $total_leave_used += ($l['leave_details'][$lt->getId()]['no_of_days_alloted'] - $l['leave_details'][$lt->getId()]['no_of_days_available']);
                                }else{
                                    echo 0;
                                }
                                $total_leave_credits += $l['leave_details'][$lt->getId()]['no_of_days_alloted'];
                            ?>
                        </td> 
                <?php } ?>
            <?php } ?>
