...
$("#date_time_event").datepicker({dateFormat:'yy-mm-dd',changeMonth:true,changeYear:true,showOtherMonths:true,maxDate: '+3M +10D'});
	...
	$('#applicant_reject_add_form').ajaxForm({
		success:function(o) {
				if(o==1) {
					dialogOkBox('Successfully Added',{ok_url: 'recruitment/profile?rid=<?php echo Utilities::encrypt($details->id) ?>&hash=<?php echo $details->getHash();?>&status=<?php echo REJECTED; ?>#application_history'});	
					//location.href=base_url+ 'recruitment/profile?rid=<?php echo Utilities::encrypt($details->id) ?>&hash=<?php echo $details->getHash();?>&status=<?php echo REJECTED; ?>#application_history'; 
				}else {
					dialogOkBox(o,{});	
				}
		},
...
<div>
<form id="applicant_reject_add_form"  action="<?php echo url('recruitment/_update_applicant_event'); ?>" method="post"  name="applicant_examination_form" > 
...
<input type="hidden" id="application_status" name="application_status" value="<?php echo REJECTED; ?>"  />
