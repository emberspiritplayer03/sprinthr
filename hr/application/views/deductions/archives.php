...
<div class="break-bottom inner_top_option">
    ...
    <div class="pull-right earnings-total total bold">Total:&nbsp;<span class="float-right"><span class="label label-info" id="total_archived"></span></span></div>
    <?php if($is_period_lock == G_Cutoff_Period::NO){ ?>	    
        <div class="datatable_withselect display-inline-block right-space">
            <select disabled="disabled" name="chkAction" id="chkAction" onchange="javascript:withSelectedArchived(this.value);">
                ...
                <option value="deduction_restore">Restore Archived</option>                     
            </select>
        </div>
    <?php }else{ ?>
...
<script>
	$(function() { 
		load_archived_deductions_list_dt("<?php echo $eid; ?>");
		load_sum_archived_deductions("<?php echo $eid; ?>");
	});
</script>
