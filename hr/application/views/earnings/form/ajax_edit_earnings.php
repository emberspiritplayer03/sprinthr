...
$(document).ready(function() {	
	$('#edit_earning').validationEngine({scroll:false});	
	...
	var t = new $.TextboxList('#employee_id', {
			...
			plugins: {
				autocomplete: {
					...
					onlyFromValues: true,
					queryRemote: true,
					remote: {url: base_url + 'earnings/ajax_get_employees_autocomplete'}			
				}
		}});
...
function checkForm()
{
	if ($('#edit_earning').validationEngine({returnIsValid: true })) {		
		$('#edit_earning').ajaxForm({
			success:function(o) {
				if (o.is_success == 1) {								
...
<form id="edit_earning" name="edit_earning" onsubmit="javascript:checkForm();"  action="<?php echo url('earnings/_save_earning'); ?>" method="post"> 
...
<input type="hidden" id="earning_id" name="earning_id" value="<?php echo Utilities::encrypt($gee->getId()); ?>" />      
    <div id="form_default">      
        <table>
        	 <tr>
             ...
             <tr>
               ...
               <td>
                    ...
                    <label class="checkbox">
                    	<input <?php echo($gee->getApplyToAllEmployee() == G_Employee_Earnings::YES ? 'checked="checked"' : ''); ?> type="checkbox" onchange="javascript:chkEmployee();" id="apply_to_all_employee" name="apply_to_all_employee" />Apply to all Employee
                    </label>
             ...
             <tr>
               ...
               <td>
                     ...
                     <label class="checkbox inline">
                    	<input <?php echo($gee->getTaxable() == G_Employee_Earnings::YES ? 'checked="checked"' : '') ?> type="checkbox" id="is_taxable" name="is_taxable" />Taxable
                    </label>               		
               </td>
             </tr>
             <tr>
             ...
             <tr>
               ...
               <td>
               		...
               		<option <?php echo($gee->getStatus() == G_Employee_Earnings::PENDING ? 'selected="selected"' : '') ?> value="<?php echo G_Employee_Earnings::PENDING; ?>" selected="selected"><?php echo G_Employee_Earnings::PENDING; ?></option>  
                    <option <?php echo($gee->getStatus() == G_Employee_Earnings::APPROVED ? 'selected="selected"' : '') ?> value="<?php echo G_Employee_Earnings::APPROVED; ?>"><?php echo G_Employee_Earnings::APPROVED; ?></option>                  
                    </select>
               </td>
             </tr>
             <tr>
