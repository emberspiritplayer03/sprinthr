...
function get_main_base_url()
{
	return 'http://' . $_SERVER['HTTP_HOST'] . MAIN_FOLDER;
}

function main_url($url)
{
	return 'http://' . $_SERVER['HTTP_HOST'] . MAIN_FOLDER . 'index.php/' . $url;
}

function hr_schedule_url($url)
{
	return 'http://' . $_SERVER['HTTP_HOST'] . HR_SCHEDULE_BASE_FOLDER . 'index.php/' . $url;
}
