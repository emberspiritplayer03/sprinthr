...
class Reports_Controller extends Controller
{
	...
	function _load_sss_r1a()
	{
		...
		$this->var['action'] 	= 'reports/download_sss_report';
	...
	function download_sss_report() {
		...
		$total 		= G_Employee_Helper::countTotalPayslipIsNotArchiveDateRange($_POST['date_from'],$_POST['date_to']);
		$pages		= floor($total/40);
		$employee_counter = 0;
		
		if($_POST['date_from'] != "" && $_POST['date_to'] != "") {
			$filename = "SSS_R-1A_".date("m-d-Y",strtotime($_POST['date_from']))."_".date("m-d-Y",strtotime($_POST['date_to']));
		} else {
			$filename = "SSS_R-1A_".date("Y-m-d");
		}
		
		if($_POST['page_number'] == 'all') {
			$this->var['filename'] 		= $filename .= "_allpages";
			for($i=1;$i<=$pages;$i++){ 
				$limit_start += 40; 
				$date = Tools::getGmtDate('Y-m-d');
				$this->var['current_page'] 	= $i;
				$this->var['total_pages']	= $pages;
				$this->var['payslip'] 		= $payslip = G_Employee_Finder::findByPayslipDateRangeIsNotArchive($_POST['date_from'], $_POST['date_to'],$limit_start . ',40');
				$this->var['total_records'] = $total;
				$this->view->render('reports/contribution/sss/download_sss_report.html.php',$this->var);
			}
		} else {
			for($i=1;$i<=$_POST['page_number'];$i++){ 
				$limit_start += 40; 
			}
			$this->var['filename'] 		= $filename .= "_p".$_POST['page_number'];
	...
	function payroll_register() {
		...
		$this->var['employees'] = $employees = G_Employee_Finder::findByPayslipPeriodAndIsNotArchive($from, $to);	
	...
	function _load_total_pages()
	{
		...
		$this->view->render('reports/pagibig/_total_pages.php', $this->var);
	}
	...
	function _load_philhealth_total_pages()
	{
		...
		$pages = floor($count/40);
		$this->var['pages'] = $pages;
		$this->view->render('reports/philhealth/_total_pages.php', $this->var);
	}
	...
	{	
		...
		if($info){
			...
			$total_employees = G_Employee_Helper::countTotalPayslipDateRange($_POST['pagibig_date_start'],$_POST['pagibig_date_to']);
			...
			if($_POST['limit_start'] == 'all'){
				 ...
				 for($i=1;$i<=$pages;$i++){ 
					...
					$this->var['filename']  = 'pagibig_monitoring.xls';
				 	$this->var['employees'] = G_Employee_Finder::findByPayslipDateRange($_POST['pagibig_date_from'], $_POST['pagibig_date_to'],$limit_start . ',40');			
					$this->view->render('reports/pagibig/pagibig_download.php', $this->var);
					//$this->view->render('benchmark/bio/pagibig_download_noheader.php', $this->var);
				 }				
			}else{
				$this->var['filename']  = 'pagibig_monitoring.xls';
				$this->var['employees'] = G_Employee_Finder::findByPayslipDateRange($_POST['pagibig_date_from'], $_POST['pagibig_date_to'],$_POST['limit_start'] . ',40');			
				$this->view->render('reports/pagibig/pagibig_download.php', $this->var);
			}
		}
	}
	...
	{	
		...
		if($info){
			...
			$total_employees = G_Employee_Helper::countTotalPayslipDateRange($_POST['philhealth_date_from'],$_POST['philhealth_date_to']);			
			$pages 		  = floor($total_employees/40);
			...
			if($_POST['philhealth_limit_start'] == 'all'){ 				
				 for($i=1;$i<=$pages;$i++){ 
					...
					$this->var['filename']  	  = 'philhealth.xls';
				 	$this->var['employees'] = G_Employee_Finder::findByPayslipDateRange($_POST['philhealth_date_from'], $_POST['philhealth_date_to'],$limit_start . ',40');			
					$this->view->render('reports/philhealth/philhealth_download.php', $this->var);					
				 }				
			}else{
				$this->var['filename']  	  = 'philhealth.xls';
				$this->var['employees'] = G_Employee_Finder::findByPayslipDateRange($_POST['philhealth_date_from'], $_POST['philhealth_date_to'],$_POST['philhealth_limit_start'] . ',40');			
				$this->view->render('reports/philhealth/philhealth_download.php', $this->var);
			}
		}
	}
