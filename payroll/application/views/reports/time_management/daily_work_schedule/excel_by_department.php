...
foreach ($data as $key => $val) {
	...
	$pe->write($start_row, 0, $department, $text);
	$pe->write($start_row, 1, $val['employee_code'], $text);
	$pe->write($start_row, 2, $val['employee_name'], $text);
	$pe->write($start_row, 3, $val['schedule_name'], $text);
	$pe->write($start_row, 4, $val['time_in'], $text);
	$pe->write($start_row, 5, $val['time_out'], $text);
