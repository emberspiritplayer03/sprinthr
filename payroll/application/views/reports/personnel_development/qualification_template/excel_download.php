...
$pe->getActiveSheet()->setTitle('Applicant');

//foreach ($data as $e) {
//	$candidate[$e['id']]['applicant'] = $e;
//}

$pe->write(0, 0, 'Applicant By Schedule', $header);
...
$pe->write(3, 4, 'Event Type', $field_name);
$pe->write(3, 5, 'Date', $field_name);
$pe->write(3, 6, 'Assigned By', $field_name);
$pe->write(3, 7, 'Position Applied', $field_name);
...
foreach ($data as $key => $val) {
	...
	$pe->write($start_row, 4, $GLOBALS['hr']['application_event'][$val['event_type']], $text);
	$pe->write($start_row, 5, Date::convertDateIntIntoDateString($val['date_time_event']), $text);
	$pe->write($start_row, 6, $val['hiring_manager'], $text);
	$pe->write($start_row, 7, $val['position_applied'], $text);
	$start_row++;
}
