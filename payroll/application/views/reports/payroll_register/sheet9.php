...
$pe->write(1, 0, "ACCUMULATED CASH BOND", $header);
...
foreach ($employee as $employee_id => $values) {
	...
	foreach ($dates as $payout_date => $value) {
		...
		if ($obj_accumulated_payslip) {
			$ph_cash_bond = new G_Payslip_Helper($obj_accumulated_payslip);
			$emp[$counter][$payout_date] = (float) $ph_cash_bond->getValue('cash bond');
