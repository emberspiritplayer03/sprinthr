...
$pe->write(1, 0, "Employees SSS Monitoring", $header);
...
<table  width="100%" border="1" cellpadding="2" cellspacing="1" style="font-size:8pt; width:836pt; line-height:16pt;">
    ...
    <tr>
    	<td colspan="3">Employees SSS Monitoring</td>
...
foreach ($employee as $employee_id => $values) {
	...
	$emp[$counter]['sss_number'] = $e->getSssNumber();
	$emp[$counter]['ee'] = $ph->getValue('sss');
	$emp[$counter]['er'] = $ph->getValue('sss_er');
?>
 	<tr>
     	<td><?php echo $e->getEmployeeCode(); ?></td>
        <td><?php echo $e->getName(); ?></td>
        <td><?php echo $e->getSssNumber(); ?></td>
        <td><?php echo $ph->getValue('sss');?></td>
        <td><?php echo $ph->getValue('sss_er');?></td>
    </tr>
...
foreach($emp as $key => $value){	
	...
	$pe->write($start_row,2,$emp[$key]['sss_number'],$data); //col 2 position
