...
class Benchmark_Bio_Controller extends Controller
{
	...
	function dropIndexesRebuildIndexes()
	{
		...
		$data = $sprint->rebuildTableIndexes();
	...
	function testDate()
	{
		$date_1 = "2015-02-20 07:33:00";
		$date_2 = "2015-02-21 07:33:00";

		$date_converted_1 = strtotime($date_1);
		$date_converted_2 = strtotime($date_2);

		echo "Date 1 : {$date_converted_1} / Date 2 : {$date_converted_2}";

	}

	function autoOvertime()
	{	
		$e = G_Employee_Finder::findById(6);		
		/*$data['date_in']  = "2015-02-20";
		$data['date_out'] = "2015-02-21";
		$data['time_in']  = "18:58:00";
		$data['time_out'] = "07:33:00";

		$data['scheduled_date_in'] = "2015-02-20";
		$data['schedule_date_out'] = "2015-02-21";
		$data['schedule_time_in']  = "19:30:00";
		$data['schedule_time_out'] = "05:15:00";*/

		/*$data['date_in']  = "2015-02-20";
		$data['date_out'] = "2015-02-20";
		$data['time_in']  = "18:58:00";
		$data['time_out'] = "20:33:00";
		...
		$data['date_in']  = "2015-02-28";
		$data['date_out'] = "2015-03-01";
		$data['time_in']  = "19:03:00";
		$data['time_out'] = "07:33:00";

		$schedule['schedule_in']  = "19:30:00";
        $schedule['schedule_out'] = "05:15:00";

        $break_schedules = $e->getEmployeeBreakTimeBySchedule($schedule);	

        Utilities::displayArray($break_schedules);

		$data['scheduled_date_in'] = "2015-02-17";
		$data['schedule_date_out'] = "2015-02-17";
		$data['schedule_time_in']  = "07:30:00";
		$data['schedule_time_out'] = "17:15:00";
	...
	function checkBreakConflictSchedule()
	{
		...
		$br = new G_Break_Time_Schedule();
		$br->setScheduleIn($schedule_in);
		$br->setScheduleOut($schedule_out);
		$br->setBreakIn($break_in);
		$br->setBreakOut($break_out);
		$data = $br->checkIfWithConflictSchedule();
	...
	{
		$addonKey = "employee_online_portal";
		$ad = new G_Sprint_Add_Ons();
		$data = $ad->getAddOnInfo($addonKey);
	...
	{	
		...
		Utilities::displayArray($arrXml);
	}

	function validateAddOn()
	{
		$addon = 'employee_online_portal6';
	...
	function updateXmlAddOns()
	{
		...
		if(Tools::isFileExist($file)==true) {
			...
			$obj[0]->employee_online_portal = 'disabled';						
			$xmlStr->asXml($xmlUrl);
		}
	}

	function triggersLocal()
	{
		/*$tables = array("g_allowed_ip","g_employee","g_leave","g_requests","g_request_approvers","g_request_approvers_level","g_request_approvers_requestors","g_roles","g_role_actions","g_employee","g_employee_attachment","g_employee_attendance","g_employee_attendance_correction_request","g_employee_basic_salary_history","g_employee_benefits","g_employee_benefits_main","g_employee_branch_history","g_employee_change_schedule_request","g_employee_contact_details","g_employee_contribution","g_employee_deductions","g_employee_job_history","g_employee_leave_available","g_employee_leave_request","g_employee_overtime","g_employee_overtime_request","g_employee_payslip","g_employee_subdivision_history","g_employee_user","g_employee_official_business_request");*/
		...
		echo "{$count} TRIGGERS was successfully created!";
	}
	...
	function triggersLive()
	{
		...
		foreach( $tables as $table_name ){
			...
			$sql ="				
				CREATE DEFINER=`root`@`localhost` TRIGGER `trig_{$table_name}_au` AFTER UPDATE ON `{$table_name}` 
				FOR EACH ROW BEGIN
					IF( (SELECT COUNT(id) FROM g_sync_data WHERE table_name = '{$table_name}' AND pk_id_local = 0 AND pk_id_live = NEW.id AND ACTION = \"update\") <= 0 )
					THEN
						INSERT INTO g_sync_data
						SET table_name = '{$table_name}',
						pk_id_local = 0,
			...
			$sql = "				
				CREATE DEFINER=`root`@`localhost` TRIGGER `trig_{$table_name}_ad` AFTER DELETE ON `{$table_name}` 
				FOR EACH ROW BEGIN
					IF( (SELECT COUNT(id) FROM g_sync_data WHERE table_name = '{$table_name}' AND pk_id_local = 0 AND pk_id_live = OLD.id AND ACTION = \"delete\") <= 0 )
					THEN
						INSERT INTO g_sync_data
						SET table_name = '{$table_name}',
						pk_id_local = 0,
	...
	function taxAlpha()
	{
		...
		echo "Total Taxable Compensation : {$total_taxable_compensation} / Total Net Compensation : {$net_taxable_compensation}";

	}

	function netTaxableData() 
	{		
		$net_compensation = 460600.00;
		$nt = new G_Net_Taxable_Table();
		$nt->setNetTaxableCompensation($net_compensation);
		$data = $nt->getTaxableCompensationDataByNetCompensation();
	...
	function taxRefund()
	{
		$tax_compensation = 460600;
		$tax_withheld     = 118082;
		$nt = new G_Net_Taxable_Table();
		$nt->setNetTaxableCompensation($tax_compensation);
		$nt->setWithholdingTax($tax_withheld);
		$data = $nt->getTaxDue();
	...
	function sendRequestENotification()
	{
		$request_id   = 1;
	...
	function updateRequestApprovers()
	{
		$id = 7;
		$approvers  = array(
			0 => "4,5",
		...
		$data = $gr->updateRequestApprovers();
	...
	function createUserUnitialFile()
	{
		$new_session_id = session_id();
		$filename = TEMP_USER_FOLDER . 'ini_' . $new_session_id;			
	...
	function generateCutoffPeriodsByDate()
	{
	    $number_of_cutoff_to_generate = 1; //previous cutoff

		for( $counter = 0; $counter < $number_of_cutoff_to_generate+1; $counter++ ){
			$date = date("2016-03-01",strtotime("-{$counter} months"));
			$c = new G_Cutoff_Period();
			$return = $c->generateCutOffPeriodsByDate($date);
		}		
	}

	function importWikiDatabase()
	{
		...
		$return = $mdb->importWikiDatabase($sql_file);
	}
	...
	function is_connected()
	{
	    ...
	    if ($connected){
	        $is_connected = "Connection successful"; //action when connected
	        fclose($connected);
	    }else{
	        $is_connected = "Connection was not successful"; //action in connection failure
	    }
	...
	function applyVersionChangesToSprintDatabase()
	{
		$version = '1.2.0017';
		$db = new Sprint_Tables();
		$logs = $db->updateTablesByAppVersion($version);
		if( $logs['is_success'] ){
			$v = new G_Sprint_Version();
			$v->updateVersionTextFile($version);
		}

		Utilities::displayArray($logs);
	}
	...
	function versionCheck()
	{
		...
		$version1     = $version_part[0];

		if( $version1 > $version2 ){
			echo "{$version1} Higher";
		}else{
			echo "{$version2} Higher";
		}
	}

	function versionFileInfo()
	{
		$data  	 = array();				
		$v = new G_Sprint_Version();
		$data = $v->getAppVersion();
	...
	function updateVersionFileInfo()
	{
		$data  	 = array();		
		$version = '1.1.0011';
		$v = new G_Sprint_Version();
		$data = $v->updateVersionTextFile($version);
		echo $data;
	}
