...
$(document).ready(function() {
	...
	$('#login_form').ajaxForm({
		success:function(o) {
			if (o.is_success == 1) {
				window.location = base_url+'audit_login<?php echo $url_param; ?>';
			} else {
...
<div id="login_wrapper">
    <div id="login_container">
        ...
        <?php 
		if (!$company_structure_id_audit || !$username_audit || !$employee_id_audit ) { ?>
        	
            <form name="login_form" id="login_form" class="login_form" method="post" action="<?php echo url('audit_login/_login'); ?>">
            ...
            <?php }else {?> 
			 ...
			 <form id="login_form" method="post" action="<?php echo url('login/_login'); ?>">
            	<div class="top_title"><a class="loggedin_actionout" href="<?php echo BASE_FOLDER; ?>index.php/audit_login/logout"><i class="icon-off icon-white"></i>  Logout</a><img src="<?php echo BASE_FOLDER; ?>themes/<?php echo THEME; ?>/themes-images/login/account_login_title.png" border="0" /></div>
                <div class="bottom_div"></div>
                <div class="gleent_logo"></div>
                <label class="input_container loggedin">
                <div align="center" class="user_selection">
       			<div align="center" style="font-size:16px;">Welcome! <strong><?php echo $_SESSION['sprint_hr_audit']['username']; ?></strong></div><br />
            		<?php foreach($mod as $key =>$value) { ?>
                            <?php if($value=='audit' && MOD_AUDIT_TRAIL == true) { ?>
                            <a class="blue_button" href="<?php echo BASE_FOLDER; ?>index.php/audit_trail/"><i class="icon-user icon-white"></i> Audit Trail</a>
                            <?php } ?>
                  	<?php } ?>
                    <div class="clear"></div>
                </div>
