...
function _editAttendanceLog(eid, type, events) {
	...
	$.get(base_url + 'attendance/ajax_edit_attendance_log', {eid:eid, type:type}, function(data) {
		closeDialog(dialog_id);
		$(dialog_id).html(data);
		dialogGeneric(dialog_id, {
			title: title,
	...
	$dialog.dialog({
		...
		buttons: {
			'Yes' : function(){
				...
				$dialog.hide();
				disablePopUp();
				$.post(base_url+'attendance/_load_delete_logs',{id:id, type: type},
					function(o){													
...
function _withSelectedLogs(logs, action, events) {
	if (action == 'update') {
		...
		$.post(base_url + 'attendance/ajax_batch_edit_attendance_log', {logs:logs}, function(data) {
			closeDialog(dialog_id);
			$(dialog_id).html(data);
			dialogGeneric(dialog_id, {
				title: title,
			...
			$dialog.dialog({
				...
				buttons: {
					'Yes' : function(){
						...
						$dialog.hide();
						disablePopUp();
						$.post(base_url+'attendance/ajax_batch_delete_attendance_log', {logs:logs},
							function(o){													
