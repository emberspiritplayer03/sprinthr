...
function load_attendance_logs_list_dt() {
	...
	$.post(base_url + 'attendance/_load_attendance_logs_dt',{limit:limit,date_from:date_from,date_to:date_to,error_type:error_type,emp_sel:emp_sel,filter:filter},
	function(o) {
...
function load_attendance_logs_list_dt() {
	...
	$('#loading_wrapper').html('<div id="dt_processing">' + loading_image + '</div>');
	$.post(base_url + 'attendance/_load_attendance_logs_dt',{limit:limit,date_from:date_from,date_to:date_to,error_type:error_type,emp_sel:emp_sel,filter:filter, device_id},
	function(o) {
		$('#loading_wrapper').html('');
		$('#attendance_logs_dt_wrapper').html(o.table);
		$('.paginator').html(o.paginator)
...
function download_attendance_log() {
	...
	if(emp_sel != ""){
		...
		function(o) {
			location.href = base_url + 'attendance/download_logs?from=' + date_from + '&to=' + date_to + '&error_type=' + error_type + '&emp=' + o.did +'&device_id='+device_id;				
		},"json");		
	}else{
		location.href = base_url + 'attendance/download_logs?from=' + date_from + '&to=' + date_to + '&error_type=' + error_type  +'&device_id='+device_id;				
	}
