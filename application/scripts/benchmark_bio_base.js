...
function load_company_info() {
	$('#c-info').html('Loading...');	
	$.post(base_url + 'startup/_load_company_info',{},
		function(o){			
			$('#c-info').html(o);		
		});		
}
...
function load_edit_company_info() {
	$("#company_profile_form").html('Loading...');
	$.post(base_url + 'startup/_load_edit_company_info',{},function(o) {
...
function load_department() {
	$('#c-department').html('Loading...');	
	$.post(base_url + 'startup/_load_department_startup',{},
		function(o){			
			$('#c-department').html(o);		
		});		
}
...
function load_employee() {
	$('#c-employee').html('Loading...');	
	$.post(base_url + 'startup/_load_employee',{},
		function(o){			
			$('#c-employee').html(o);		
		});		
}
...
//			}else {
				$.post(base_url+"startup/_load_add_branch_form",{},
				function(o){
...
function checkForAddBranchDepartmentStartup(add) 
{
	...
	if(add=='add') {
			$.post(base_url+"startup/_load_add_branch_form_startup",{},
			function(o){
...
function load_employee_datatable(searched)
{
	...
	var action = function(elCell, oRecord, oColumn, oData) { 
				...
				elCell.innerHTML = "<div id='dropholder'><a class='dropbutton' href='startup/profile?eid="+id+"&hash="+hash+"#personal_details'>Display</a></div>"; 
		};
	...
	var employee_name_f = function(elCell, oRecord, oColumn, oData) { 
				...
				if(status=='Terminated' || status=='End of Contract' || status=='AWOL') {
					elCell.innerHTML = "<a class='dropbutton' href='startup/profile?eid="+id+"&hash="+hash+"#personal_details'><font color='red'><img class='images_wrapper' src='"+photo+"' height='80' style='display:none' align='left'  />&nbsp;"+employee_name+"</font></a>"; 	
				}else {
					elCell.innerHTML = "<a class='dropbutton' href='startup/profile?eid="+id+"&hash="+hash+"#personal_details'><img class='images_wrapper' src='"+photo+"' height='80' style='display:none' align='left'  />&nbsp;"+employee_name+"</a>"; 	
				}
...
function load_schedule_startup() {
	$('#c-default-schedule').html('Loading...');	
...
{
	$("#grace_period_dropdown_wrapper_startup").html(loading_image+' loading...');
...
{
	$("#c-default-leave").html(loading_image+' loading...');
...
function load_payroll_settings_startup() {
	$('#c-payroll-settings-startup').html('Loading...');	
...
function load_deduction_breakdown_list() {
	$("#deduction_breakdown_list_wrapper").html('Loading...');
...
function load_add_new_pay_period() {
	blockPopUp();
	$("#payperiod_form").html('Loading...');
	$.post(base_url + 'startup/_load_add_new_pay_period',{},function(o) {
...
function load_edit_pay_period(pay_period_id) {
	blockPopUp();
	$("#payperiod_form").html('Loading...');
	$.post(base_url + 'startup/_load_edit_pay_period',{pay_period_id:pay_period_id},function(o) {
		$("#payperiod_form").html(o);								 
	})	
	var $dialog = $('#payperiod_form');
	 $dialog.dialog("destroy");
