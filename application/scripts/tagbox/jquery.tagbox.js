...
//  Hide Element
  ...
  //  Create DOM Elements
      if( (options.enableDropdown) && options.dropdownSource() != null ) {
        ...
        var $tag_input_elem = (options.readonly) ? '' : jQuery('<input type="text" class="tipsy_input '+options.tagInputClassName+' '+options.className+'-input" value="" id="'+options.className+'-input-'+uuid+'" />');
      }
  ...
  //  Methods
      ...
      function generate_tags_list(tags_list) {
        ...
        jQuery.each(tags_list, function(key, val) {
          if(val != "") {
            var remove_tag_link = (options.readonly) ? '' : '<a href="javascript:void(0)" class="'+options.className+'-remove '+options.className+'-remove-'+uuid+'" title="Remove Tag" rel="'+val+'">'+options.removeTagText+'<input type="hidden" name="tag[]" value="' + val + '"/></a>';
