...
class G_Payslip_Manager {
    ...
    public static function saveMultiple($payslips) {        
        ...
        if ($has_record) {            
            ...
            $sql_insert = "
                INSERT INTO ". G_EMPLOYEE_PAYSLIP ." (id, employee_id, period_start, period_end, basic_pay, tardiness_amount, number_of_declared_dependents, overtime, gross_pay,
                                                        ...
                                                        month_13th, sss, pagibig, philhealth)
	...
	public static function saveOLD($payslip) {
		...
		if ($payslip->getId() == 0) {
			$sql_start = "INSERT INTO g_employee_payslip";
			...
			$sql_start = "UPDATE g_employee_payslip";
    ...
    public static function delete(G_Payslip $p){
        if(G_Payslip_Helper::isIdExist($p) > 0){
            $sql = "
                DELETE FROM g_employee_payslip 
