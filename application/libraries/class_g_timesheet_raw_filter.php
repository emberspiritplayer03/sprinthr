...
class G_Timesheet_Raw_Filter {
    ...
    public function filterAndUpdateAttendance() {
        $timesheets = $this->timesheets;
        ksort($timesheets);
        foreach ($timesheets as $employee_id => $timesheet) {
            ksort($timesheet);
        ...
        G_Attendance_Manager::recordToMultipleEmployees($this->all_attendance);
    }
		...
		foreach ($ins as $date_in => $times) {
			...
			if ($date_time_out) {
				...
				if ($is_add) {
					...
					$this->addTimesheet($employee_id, $date_in, $time_in, $date_out, $time_out, $breaks);
...
//		echo "<br>======<br>";
	...
	protected function addTimesheet($employee_code, $date_in, $time_in, $date_out, $time_out, $breaks = array()) {
		$e = G_Employee_Finder::findByEmployeeCode($employee_code);
		if ($e) {
			$is_true = G_Attendance_Helper::recordTimecard($e, $date_in, $time_in, $time_out, $date_in, $date_out, null, null, 0, $breaks);
			if ($is_true) {
                $this->all_attendance[] = G_Attendance_Helper::generateAttendance($e, $date_in);
			}
		} //else {
