...
class G_Account_Import {
			...
			$cellIterator = $row->getCellIterator();
		   	...
		   	foreach ($cellIterator as $cell) {				
				...
				if ($column == 'B' && $cell_value != '') {
					$this->module = $cell_value;					
				}
				
				if ($column == 'C' && $cell_value != '') {
					$this->username = ucfirst($cell_value);					
				}
				
				if ($column == 'D' && $cell_value != '') {
					$this->password = $cell_value;
				}
				
				if ($this->employee_code != '' && $this->employee_code != 'Employee Code' && $this->username != '' && $this->password != '' && $this->module != '') {					
					$is_saved = $this->save();
					if ($is_saved) {
						$is_imported = true;	
					}
	...
	protected function save() {				
		...
		if($e) {
			$is_user_exist = G_User_Finder::findByEmployeeId($e->getId());
			if($is_user_exist){
				
			}else{
				$is_exist = G_User_Helper::isUsernameExist($this->username);
				if($is_exist){
					
				}else{
					$j = G_Employee_Job_History_Finder::findCurrentJob($e);
					$u = new G_User;
					$u->setCompanyStructureId($this->company_structure_id);
					$u->setEmployeeId($e->getId());
					$u->setEmploymentStatus($j->getEmploymentStatus());
					$u->setHash($e->getHash());
					$u->setUsername($this->username);
					$u->setPassword(Utilities::encryptPassword($this->password));
					$u->setModule($this->module);
