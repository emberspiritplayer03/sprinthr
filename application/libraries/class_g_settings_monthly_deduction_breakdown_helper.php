<?php
class G_Settings_Monthly_Deduction_Breakdown_Helper {
    ...
    public static function getDeductionPercentage($cutoff_number, $deduction_type) {
        $sdb = G_Settings_Monthly_Deduction_Breakdown_Finder::findByIdAndIsActive($deduction_type);
        if($sdb){
	...
	public static function isIdExist(G_Settings_Monthly_Deduction_Breakdown $e) {
		$sql = "
			SELECT COUNT(*) as total
			FROM " . G_SETTINGS_MONTHLY_DEDUCTION_BREAKDOWN ."
	...
	public static function sqlGetAllActiveDeductionBreakDown() {
		$sql = "
			SELECT * 
			FROM " . G_SETTINGS_MONTHLY_DEDUCTION_BREAKDOWN ."		
			WHERE is_active =". Model::safeSql(G_Settings_Monthly_Deduction_Breakdown::YES) . "	
		";
	...
	public static function getCurrentPayPeriodPercentageDeductedByEmployeedAndDeductible(G_Employee $e,$deductible,$end_date){
		...
		$sdb			 = G_Settings_Monthly_Deduction_Breakdown_Finder::findByIdAndIsActive($deductible);
		if($sdb){
			$d_percentage = $sdb->getBreakdown();
			$d_percentage = explode(":",$d_percentage);
