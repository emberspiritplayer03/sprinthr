<?php
class G_Settings_Weekly_Deduction_Breakdown_Helper {

    /*
     * @param $deduction_type G_Settings_Weekly_Deduction_Breakdown::SSS, G_Settings_Weekly_Deduction_Breakdown::HDMF, G_Settings_Weekly_Deduction_Breakdown::PHIL_HEALTH
     */
    public static function getDeductionPercentage($cutoff_number, $deduction_type) {
        $sdb = G_Settings_Weekly_Deduction_Breakdown_Finder::findByIdAndIsActive($deduction_type);
        if($sdb){
	...
	public static function isIdExist(G_Settings_Weekly_Deduction_Breakdown $e) {
		$sql = "
	...
	public static function sqlGetAllActiveDeductionBreakDown() {
		$sql = "
			SELECT * 
			FROM " . G_SETTINGS_WEEKLY_DEDUCTION_BREAKDOWN ."		
			WHERE is_active =". Model::safeSql(G_Settings_Weekly_Deduction_Breakdown::YES) . "	
		";
	...
	public static function getCurrentPayPeriodPercentageDeductedByEmployeedAndDeductible(G_Employee $e,$deductible,$end_date){
		...
		$sdb			 = G_Settings_Weekly_Deduction_Breakdown_Finder::findByIdAndIsActive($deductible);
		if($sdb){
			$d_percentage = $sdb->getBreakdown();
			$d_percentage = explode(":",$d_percentage);
