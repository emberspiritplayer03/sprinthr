<?php
class G_Weekly_Payslip_Generator implements IGenerator {
    ...
    protected $frequency_id = 2;

    /*
     * @param array $employees array instance of G_Employee
     */
    public function __construct(G_Weekly_Cutoff_Period $period, $employees = '') {
    ...
    public function save($payslips) {        
        return G_Weekly_Payslip_Manager::saveMultiple($payslips);
    }
    ...
    */
    ...
    private function generateByEmployee(IEmployee $e) {
        ...
        $p = G_Weekly_Payslip_Finder::findByEmployeeAndPeriod($e, $start_date, $end_date);
        if (!$p) {
            $p = new G_Weekly_Payslip;
        }
        ...
        $cutoffStartDateArray = explode("-",$cutoffa);
        ...
        //test for leave with pay
        $present_days_with_pay +=  $total_days_leave_with_pay;
        ...
        $regular_amount = G_Weekly_Payslip_Helper::computeRegularAmount($a, $per_day, $per_hour);
        if($is_disable_computation) {
            $regular_amount = 0;
        }
        ...
        $sv = G_Sprint_Variables_Finder::findByVariableName(G_Sprint_Variables::FIELD_DEFAULT_WEEKLY_PAYROLL_RATES);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $regular_ot_amount = G_Weekly_Payslip_Helper::computeRegularOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);

            $cut_regular_ot_amount = G_Weekly_Payslip_Helper::computeCutRegularOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);
            $prev_regular_ot_amount = G_Weekly_Payslip_Helper::computePrevRegularOvertimeAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
        ...
        $regular_ns_amount = G_Weekly_Payslip_Helper::computeRegularNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);


        $cut_regular_ns_amount = G_Weekly_Payslip_Helper::computeCutRegularNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        $prev_regular_ns_amount = G_Weekly_Payslip_Helper::computePrevRegularNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $regular_ns_ot_amount = G_Weekly_Payslip_Helper::computeRegularOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);



            $cut_regular_ns_ot_amount = G_Weekly_Payslip_Helper::computeCutRegularOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
            $prev_regular_ns_ot_amount =  G_Weekly_Payslip_Helper::computePrevRegularOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $restday_amount = G_Weekly_Payslip_Helper::computeRestDayAmount($a, $rate, $per_day, $per_hour, $custom_ot, $mandated_status);//$c->computeRestDay();

            $prev_restday_amount = G_Weekly_Payslip_Helper::computePrevRestDayAmount($a, $rate, $prev_per_day, $prev_per_hour, $custom_ot, $mandated_status);
            $cut_restday_amount =  G_Weekly_Payslip_Helper::computeCutRestDayAmount($a, $rate, $per_day, $per_hour, $custom_ot, $mandated_status);          
        }
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $restday_ot_amount = G_Weekly_Payslip_Helper::computeRestDayOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);

            $prev_restday_ot_amount = G_Weekly_Payslip_Helper::computePrevRestDayOvertimeAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_restday_ot_amount = G_Weekly_Payslip_Helper::computeCutRestDayOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        }          
        ...
        //
        $restday_ns_amount = G_Weekly_Payslip_Helper::computeRestDayNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeRegularNightShift();

        $prev_restday_ns_amount = G_Weekly_Payslip_Helper::computePrevRestDayNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
        $cut_restday_ns_amount = G_Weekly_Payslip_Helper::computeCutRestDayNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $restday_ns_ot_amount = G_Weekly_Payslip_Helper::computeRestDayOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeRestDayNightShiftOvertime();
            $prev_restday_ns_ot_amount =   G_Weekly_Payslip_Helper::computePrevRestDayOvertimeNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_restday_ns_ot_amount =  G_Weekly_Payslip_Helper::computeCutRestDayOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $restday_special_amount = G_Weekly_Payslip_Helper::computeRestDaySpecialAmount($a, $rate, $per_day, $per_hour, $custom_ot, $mandated_status);

            $prev_restday_special_amount = G_Weekly_Payslip_Helper::computePrevRestDaySpecialAmount($a, $rate, $prev_per_day, $prev_per_hour, $custom_ot, $mandated_status);
            $cut_restday_special_amount = G_Weekly_Payslip_Helper::computeCutRestDaySpecialAmount($a, $rate, $per_day, $per_hour, $custom_ot, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $restday_special_ot_amount = G_Weekly_Payslip_Helper::computeRestDaySpecialOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);

            $prev_restday_special_ot_amount = G_Weekly_Payslip_Helper::computePrevRestDaySpecialOvertimeAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_restday_special_ot_amount = G_Weekly_Payslip_Helper::computeCutRestDaySpecialOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $restday_special_ns_amount = G_Weekly_Payslip_Helper::computeRestDaySpecialNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeRegularNightShift();

            $prev_restday_special_ns_amount = G_Weekly_Payslip_Helper::computePrevRestDaySpecialNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_restday_special_ns_amount =  G_Weekly_Payslip_Helper::computeCutRestDaySpecialNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            //startdito
            $restday_special_ns_ot_amount = G_Weekly_Payslip_Helper::computeRestDaySpecialOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
            
            $prev_restday_special_ns_ot_amount =G_Weekly_Payslip_Helper::computePrevRestDaySpecialOvertimeNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_restday_special_ns_ot_amount = G_Weekly_Payslip_Helper::computeCutRestDaySpecialOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $restday_legal_amount = G_Weekly_Payslip_Helper::computeRestDayLegalAmount($a, $rate, $per_day, $per_hour, $mandated_status);
            //herena
            $prev_restday_legal_amount = G_Weekly_Payslip_Helper::computePrevRestDayLegalAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_restday_legal_amount = G_Weekly_Payslip_Helper::computeCutRestDayLegalAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        }   
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $restday_legal_ot_amount = G_Weekly_Payslip_Helper::computeRestDayLegalOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);

            $prev_restday_legal_ot_amount = G_Weekly_Payslip_Helper::computePrevRestDayLegalOvertimeAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_restday_legal_ot_amount = G_Weekly_Payslip_Helper::computeCutRestDayLegalOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $restday_legal_ns_amount = G_Weekly_Payslip_Helper::computeRestDayLegalNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeRegularNightShift();

            $prev_restday_legal_ns_amount = G_Weekly_Payslip_Helper::computePrevRestDayLegalNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_restday_legal_ns_amount = G_Weekly_Payslip_Helper::computeCutRestDayLegalNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
           //last
        }  
        if($check2631){
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $restday_legal_ns_ot_amount = G_Weekly_Payslip_Helper::computeRestDayLegalOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);

            $prev_restday_legal_ns_ot_amount = G_Weekly_Payslip_Helper::computePrevRestDayLegalOvertimeNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_restday_legal_ns_ot_amount = G_Weekly_Payslip_Helper::computeCutRestDayLegalOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        }        
        ...
        switch ($salary_type):
            case G_Employee_Basic_Salary_History::SALARY_TYPE_MONTHLY:  
                $holiday_special_amount = G_Weekly_Payslip_Helper::computeSpecialAmount($a, $rate, $per_day, $per_hour, $custom_ot, $custom_ot_disapproved, $mandated_status);

                $prev_holiday_special_amount = G_Weekly_Payslip_Helper::computePrevSpecialAmount($a, $rate, $prev_per_day, $prev_per_hour, $custom_ot, $custom_ot_disapproved, $mandated_status);
                $cut_holiday_special_amount = G_Weekly_Payslip_Helper::computeCutSpecialAmount($a, $rate, $per_day, $per_hour, $custom_ot, $custom_ot_disapproved, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $holiday_special_ot_amount = G_Weekly_Payslip_Helper::computeSpecialOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeHolidaySpecialOvertime();

            $prev_holiday_special_ot_amount =  G_Weekly_Payslip_Helper::computePrevSpecialOvertimeAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_holiday_special_ot_amount =  G_Weekly_Payslip_Helper::computeCutSpecialOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        ...
        $holiday_special_ns_amount = G_Weekly_Payslip_Helper::computeSpecialNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);

        $prev_holiday_special_ns_amount = G_Weekly_Payslip_Helper::computePrevSpecialNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
        $cut_holiday_special_ns_amount = G_Weekly_Payslip_Helper::computeCutSpecialNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $holiday_special_ns_ot_amount = G_Weekly_Payslip_Helper::computeSpecialOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);

            $prev_holiday_special_ns_ot_amount = G_Weekly_Payslip_Helper::computePrevSpecialOvertimeNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);
            $cut_holiday_special_ns_ot_amount = G_Weekly_Payslip_Helper::computeCutSpecialOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);
        ...
        $labels[] = new Payslip_Label('Holiday Legal Hours', $holiday_legal_hours, 'holiday_legal_hours');
         ...
         switch ($salary_type):
            case G_Employee_Basic_Salary_History::SALARY_TYPE_MONTHLY:                   
                $holiday_legal_amount = G_Weekly_Payslip_Helper::computeLegalAmount($a, $rate, $per_day, $per_hour, $custom_ot, $mandated_status);//$c->computeHolidayLegal(); 
                // $holiday_legal_amount = $holiday_legal_hours * $per_hour * 2;

                $cut_holiday_legal_amount = G_Weekly_Payslip_Helper::computeCutLegalAmount($a, $rate, $per_day, $per_hour, $custom_ot, $mandated_status);


                $prev_holiday_legal_amount = G_Weekly_Payslip_Helper::computePrevLegalAmount($a, $rate, $prev_per_day, $prev_per_hour, $custom_ot, $mandated_status);

               
                break;
            case G_Employee_Basic_Salary_History::SALARY_TYPE_DAILY:              
                // $holiday_legal_amount = G_Weekly_Payslip_Helper::computeLegalAmountDoublePay($a, $rate, $per_day, $per_hour, $custom_ot, $mandated_status);//$c->computeHolidayLegal();  
                $holiday_legal_amount = G_Weekly_Payslip_Helper::computeLegalAmount($a, $rate, $per_day, $per_hour, $custom_ot, $mandated_status);//$c->computeHolidayLegal();  
                // $holiday_legal_amount = $holiday_legal_hours * $per_hour * 2;
        ...
        endswitch; 
                // $holiday_legal_amount = $holiday_legal_hours * $per_hour * 2;
        // var_dump([$holiday_legal_hours, $per_hour, 2]); exit;
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $holiday_legal_ot_amount = G_Weekly_Payslip_Helper::computeLegalOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeHolidayLegalOvertime();

            $cut_holiday_legal_ot_amount =  G_Weekly_Payslip_Helper::computeCutLegalOvertimeAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeHolidayLegalOvertime();

            $prev_holiday_legal_ot_amount = G_Weekly_Payslip_Helper::computePrevLegalOvertimeAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);//$c->computeHolidayLegalOvertime();
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $holiday_legal_ns_amount = G_Weekly_Payslip_Helper::computeLegalNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeRegularNightShift();

            $cut_holiday_legal_ns_amount =  G_Weekly_Payslip_Helper::computeCutLegalNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeRegularNightShift();
             $prev_holiday_legal_ns_amount =  G_Weekly_Payslip_Helper::computePrevLegalNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);//$c->computeRegularNightShift();
        ...
        if( array_key_exists($e->getId(), $this->employees_special_ot_rate) ){
            ...
            $holiday_legal_ns_ot_amount = G_Weekly_Payslip_Helper::computeLegalOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeHolidayLegalNightShiftOvertime();

             $cut_holiday_legal_ns_ot_amount = G_Weekly_Payslip_Helper::computeCutLegalOvertimeNightShiftAmount($a, $rate, $per_day, $per_hour, $mandated_status);//$c->computeHolidayLegalNightShiftOvertime();
             $prev_holiday_legal_ns_ot_amount = G_Weekly_Payslip_Helper::computePrevLegalOvertimeNightShiftAmount($a, $rate, $prev_per_day, $prev_per_hour, $mandated_status);//$c->computeHolidayLegalNightShiftOvertime();
        ...
        switch ($salary_type):
            case G_Employee_Basic_Salary_History::SALARY_TYPE_MONTHLY:
                //get cutoff count in a month
                $cutoffCountForMonth = G_Weekly_Cutoff_Period_Finder::_findByMonthlyCutoffsAndYear($cutoffStartDateArray[1], $cutoffStartDateArray[0]);
                if(!$cutoffCountForMonth){
                    $cutoffCountForMonth=4;
                }else{
                    $cutoffCountForMonth=count($cutoffCountForMonth);
                }
                if (Tools::isDateWithinDates($e->getHiredDate(), $start_date, $end_date) && $e->getHiredDate() != $start_date) { // pro rated
                    ...
                    $basic_pay = $salary_amount / $cutoffCountForMonth;
                } else {
                    $basic_pay = $salary_amount / $cutoffCountForMonth;
                }
                break;
        ...
        $contri = $e->getEmployeeContributionsByWeeklyCutoffNumber( $salary, $cutoff );  
        ...
        */
        ...
        $p->setPhilhealthEr($philhealth_er);
