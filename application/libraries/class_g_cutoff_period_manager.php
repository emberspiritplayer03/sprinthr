...
class G_Cutoff_Period_Manager {
	public static function savePeriod($current_year,$start, $end, $type, $payout_date, $cutoff_number) {
		...
		if ($row['total'] == 0) {
			$sql = "
				INSERT INTO ". G_CUTOFF_PERIOD ." (year_tag,period_start,period_end, salary_cycle_id, payout_date, cutoff_number)
				VALUES (
	...
	public static function save(G_Cutoff_Period $gcp) {
		if (G_Cutoff_Period_Helper::isIdExist($gcp) > 0 ) {
			$action    = "update";
			$sql_start = "UPDATE ". G_CUTOFF_PERIOD . " ";
			$sql_end   = "WHERE id = ". Model::safeSql($gcp->getId());		
		}else{
			$action    = "insert";
			$sql_start = "INSERT INTO ". G_CUTOFF_PERIOD . " ";
			$sql_end   = " ";		
		}
	...
	public static function lockPayrollPeriod(G_Cutoff_Period $gcp) {
		...
		if (G_Cutoff_Period_Helper::isIdExist($gcp) > 0 ) {
			...
			$sql_end   = "WHERE id = ". Model::safeSql($gcp->getId());		
				...
				$sql = $sql_start ."
				SET								
				is_lock =" . Model::safeSql(G_Cutoff_Period::YES) . " "				
				. $sql_end ."
				";		
			Model::runSql($sql);
		}				
	...
	public static function unLockPayrollPeriod(G_Cutoff_Period $gcp) {
		if (G_Cutoff_Period_Helper::isIdExist($gcp) > 0 ) {
			$sql_start = "UPDATE ". G_CUTOFF_PERIOD . " ";
			$sql_end   = "WHERE id = ". Model::safeSql($gcp->getId());		
				...
				$sql = $sql_start ."
				SET								
				is_lock =" . Model::safeSql(G_Cutoff_Period::NO) . " "				
				. $sql_end ."
				";		
