...
class Datatable {
			...
			//echo key($r);
			  ...
			  foreach ($cols as $key => $value){	
				if($value != ''){
					...
					} else if($value == 'leave_comments') { 
						$r[$value] = Tools::limitCharater($r[$value],60," "); 
						$r[$value] = str_replace("\n", " ", $r[$value]);
					} else if($value == 'date_start' || $value == 'date_end') {
						$r[$value] = ($r[$value] != "" ? date('M d, Y',strtotime($r[$value])) : '');
					} else if($value == 'date_modified') {
						$r[$value] = ($r[$value] != "" ? date('m/d/Y g:i a',strtotime($r[$value])) : '');
					} else if($value == 'time_in' || $value == 'time_out' || $value == 'actual_time_in' || $value == 'actual_time_out' || $value == 'time_start' || $value == 'time_end') {
	...
	public function constructDataTableForApplicant() {
			...
			//echo key($r);
			  ...
			  foreach ($cols as $key => $value){	
				if($value != ''){
					...
					if($value == 'application_status_id') {
						if($r[$value] == APPLICATION_SUBMITTED) {
							$r[$value] = 'Application Submitted';
						}elseif($r[$value] == INTERVIEW) { 
							$r[$value] = 'Interview';
						}elseif($r[$value] == JOB_OFFERED) { 
							$r[$value] = 'Offered a Job';
						}elseif($r[$value] == OFFER_DECLINED) { 
							$r[$value] = 'Declined Offer';
						}elseif($r[$value] == REJECTED) { 
							$r[$value] = 'Reject';
						}elseif($r[$value] == HIRED) { 
							$r[$value] = 'Hired';
						}
					}									
