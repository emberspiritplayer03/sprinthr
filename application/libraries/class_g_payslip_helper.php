...
class G_Payslip_Helper
{
	...
	public function sqlAllEmployeesSSSContributionByDateRangeNoDup($date_from = '', $date_to = '', $add_query){
		...
		$getSSS = G_Settings_Deduction_Breakdown_Finder::findByName('SSS');
	
		if($getSSS){
			$breakdownContainer = explode(':', $getSSS->getBreakdown());
	
			$c = G_Cutoff_Period_Finder::findByPeriod($date_from, $date_to);
    ...
    */
			...
			if ($a->isHoliday() && !$a->isOfficialBusiness()) {
				...
				if ($h && $h->isLegal()) {
					...
					if ($specific_sched || $sched) {
						...
						if (($a->isPresent() && $yesterday['is_present']  && !$a->isRestDay()) || ($a->isPresent() && $yesterday['is_leave'] == 1 && $yesterday['is_paid'] == 1 && !$a->isRestDay()) || ($a->isPresent() && $yesterday['is_ob'] == 1 && $yesterday['is_paid'] == 1  && !$a->isRestDay())) { //if both previous date and holiday is present or on leave with pay before holiday and present on holiday - double pay
							$hours_worked = $t->getTotalScheduleHours();
						...
						} elseif ((!$a->isPresent() && $yesterday['is_present']  && !$a->isRestDay()) || (!$a->isPresent() && $yesterday['is_leave'] == 1 && $yesterday['is_paid'] == 1 && !$a->isRestDay()) || ($a->isPresent() && $yesterday['is_ob'] == 1 && $yesterday['is_paid'] == 1) || $yesterday['is_restday'] && !$a->isRestDay()) {
							$hours_worked = 8;
							if ($is_with_custom_ot) 
							{
								$hours_worked = $custom_ot_hours_worked;
	...
	public static function computeRegularNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount, $mandated_status = 'Enable')
	{
		...
		if (strtolower($mandated_status) == 'enable') {
					...
					$total = (float) Tools::numberFormat($hourly_amount * $ns_rate * $ns_hours, 2);
