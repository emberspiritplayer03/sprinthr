...
class G_Weekly_Payslip_Finder {
	...
	//test
	public static function findByEmployeeAndDateRange3(IEmployee $employee, $start_date, $end_date) {
		$sql = "
			SELECT id, payout_date, period_start, period_end, basic_pay, gross_pay, total_earnings, total_deductions, net_pay, earnings, other_earnings, deductions, other_deductions, labels,
				taxable, non_taxable, withheld_tax, month_13th, sss, pagibig, philhealth, tardiness_amount
			FROM g_employee_weekly_payslip
			WHERE (employee_id = ". Model::safeSql($employee->getId()) .")
			AND (period_start >= " . Model::safeSql($start_date) . " AND period_end <= " . Model::safeSql($end_date) . ")
	...
	public static function findAllByYearWithOptions($year, $options, $fields) {
		...
		//		AND p.id IN (35588,35635,35606,35563,35609,35612,35517,35472, 35637,35619,35607,35502,35533,35658)
		// SELECT p.* FROM g_employee_weekly_payslip p LEFT JOIN g_employee e ON p.employee_id = e.id WHERE DATE_FORMAT(p.period_end,'%Y')=2019 ORDER BY p.employee_id ASC
	...
	public static function findAllByCutOffWithOptions($cutoff_date, $options, $fields) {
		...
		// $sql = "SELECT  FROM g_employee_weekly_payslip p LEFT JOIN g_employee e ON p.employee_id = e.id WHERE p.period_start ='2018-12-26' AND e.id = 3 ORDER BY p.employee_id ASC 3";
