<?php
class G_Employee_Attendance_Log_Manager_V2 {

    /*
     *  $multiple_attendance - array of G_EMPLOYEE_ATTENDANCE_V2
     */
    public static function saveMultiple($multiple_attendance) {
        ...
        if ($has_record) {
            ...
            $sql_insert = "
                INSERT INTO ". G_EMPLOYEE_ATTENDANCE_V2 ." (id, user_id, employee_code, date, time, type, remarks, employee_name)
	...
	/*
		$a - Instance of G_EMPLOYEE_ATTENDANCE_V2
	*/
	public static function save($a) {
		if ($a->getId() > 0) {
			$action = 'update';
			$sql_start_v2 = "UPDATE ". G_EMPLOYEE_ATTENDANCE_V2;
			$sql_end   = " WHERE id = ". Model::safeSql($a->getId());		
		} else {
			$action = 'insert';
			$sql_start_v2 = "INSERT INTO ". G_EMPLOYEE_ATTENDANCE_V2;
			//$sql_end   = ",public_id = ". Model::safeSql($public_id);
		}
		
		$time_in = date("H:i:s",strtotime($a->getTimeIn()));
		$time_out = date("H:i:s",strtotime($a->getTimeOut()));
		...
		$sql_v2 = $sql_start_v2 ."
			...
			actual_time_in = ". Model::safeSql($time_in) .",
			actual_time_out = ". Model::safeSql($time_out) .",
			project_site_id = ".Model::safeSql($a->getProjectSiteId()).",
			activity_id = ".Model::safeSql($a->getActivityId()).", 
			has_error = ".Model::safeSql($a->getHasError()).",
			error_message = ".Model::safeSql($a->getErrorMessage())."	
			". $sql_end ."	
		";
	...
	*/		
	...
	public static function updateUntransfferedDataByDate($date) {
		$sql = "
			UPDATE ". G_EMPLOYEE_ATTENDANCE_V2 ."
			SET is_transferred =" . Model::safeSql(G_EMPLOYEE_ATTENDANCE_V2::IS_TRANSFERRED) . "
			WHERE date =" . Model::safeSql($date) . "
				AND is_transferred =" . Model::safeSql(G_EMPLOYEE_ATTENDANCE_V2::ISNOT_TRANSFERRED) . "
		";		
		Model::runSql($sql);
		//return (mysql_affected_rows() >= 1) ? true : false ;
	}	
	...
	public static function resetLogsToNotTransferredByDateRange($date_from = '', $date_to = '') {
		...
		$sql = "
			UPDATE ". G_EMPLOYEE_ATTENDANCE_V2 ."
			SET is_transferred =" . Model::safeSql(G_EMPLOYEE_ATTENDANCE_V2::ISNOT_TRANSFERRED) . "
	...
	public static function importLogs( $file = '' ) {		
		...
		$sql 		          = "INSERT IGNORE INTO " . G_EMPLOYEE_ATTENDANCE_V2 . "(employee_code, type, date, time)VALUES";
		...
		$subSql = "
				UPDATE " . G_EMPLOYEE_ATTENDANCE_V2 . " fp
