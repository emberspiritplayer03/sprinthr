...
define('CSSC_VARS_DIR', dirname(__FILE__).'/');
...
Class CSSCompression
{
	...
	function runSpecialCompressions($prop, $val){
		...
		if ($this->options['fontweight2num'] && $prop === 'font-weight'){
			...
			if (! $fontweight2num)
				include(CSSC_VARS_DIR . 'css_compressor_font.php');
	...
	function runColorChanges($val){
		...
		if ($this->options['color-long2hex']){
			...
			static $long2hex;
			if (!$long2hex)
				include(CSSC_VARS_DIR.'css_compressor_long2hex.php');
		...
		if ($this->options['color-hex2shortcolor']){
			...
			static $hex2short;
			if (!$hex2short)
				include(CSSC_VARS_DIR.'css_compressor_hex2short.php');
