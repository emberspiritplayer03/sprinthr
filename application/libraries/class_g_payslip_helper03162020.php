...
//new
 public function sqlAllEmployeesSSSContributionByDateRangeNoDup($date_from = '', $date_to = '', $add_query){
    	...
    	$sql = "
			SELECT DISTINCT e.lastname, e.firstname, CONCAT(p.period_start, ' to ', p.period_end)AS cutoff_period, p.sss AS sss_contribution, e.sss_number, es.status,
				sss.company_share, sss.company_ec, e.employee_code,
				(SELECT csA.title FROM `g_company_structure` csA WHERE csA.id = e.section_id ORDER BY csA.id DESC LIMIT 1 )AS section_name,                 
				COALESCE(esh.name,(
                    SELECT name FROM `g_employee_subdivision_history`
...
public function sqlAllEmployeesRevisedPhilhealthContributionByDateRangeNoDup($date_from = '', $date_to = '', $add_query = ''){
    	...
    	$sql = "
			SELECT DISTINCT e.lastname, e.firstname, e.middlename, e.birthdate, CONCAT(p.period_start, ' to ', p.period_end)AS cutoff_period, p.philhealth AS philhealth_contribution, ge.philhealth_er ,e.philhealth_number,es.status,e.employee_code,
				(SELECT csA.title FROM `g_company_structure` csA WHERE csA.id = e.section_id ORDER BY csA.id DESC LIMIT 1 )AS section_name,                 
                COALESCE(esh.name,(
                    SELECT name FROM `g_employee_subdivision_history`
                    WHERE employee_id = p.employee_id 
    ...
    //new
    	...
    	$sql = "
			SELECT DISTINCT e.lastname, e.firstname, e.middlename, e.extension_name, p.period_start, p.period_end, p.pagibig AS pagibig_contribution, ge.pagibig_er,e.pagibig_number,es.status,
			(SELECT csA.title FROM `g_company_structure` csA WHERE csA.id = e.section_id ORDER BY csA.id DESC LIMIT 1 )AS section_name, e.employee_code,
                COALESCE(esh.name,(
                    SELECT name FROM `g_employee_subdivision_history`
                    WHERE employee_id = p.employee_id 
...
//  		";
 ...
 public static function computeCutLegalAmount($attendance, $rate, $daily_amount, $hourly_amount, $custom_ot = array()) {                
        $amount               = 0;
        $omit_100 			  = 100; // if legal, additional 200% pay to employee. we will compute it from 100% because the other 100% is in the regular pay already
        $holiday_legal_rate   = $rate->getHolidayLegal() - $omit_100;
        $total_hrs_worked     = 0;
 ...
 public static function computePrevLegalAmount($attendance, $rate, $daily_amount, $hourly_amount, $custom_ot = array()) {                
        $amount               = 0;
        $omit_100 			  = 100; // if legal, additional 200% pay to employee. we will compute it from 100% because the other 100% is in the regular pay already
        $holiday_legal_rate   = $rate->getHolidayLegal() - $omit_100;
        $total_hrs_worked     = 0;
    ...
    //new
 public static function computeCutLegalOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        	 $date = date("M d", strtotime($a->getDate()));
                $date_start = date("M 01", strtotime($date));
    ...
    public static function computePrevLegalOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        	 $date = date("M d", strtotime($a->getDate()));
                $date_start = date("M 25", strtotime($date));
    ...
    public static function computeSpecialAmount($attendance, $rate, $daily_amount, $hourly_amount, $custom_ot = array(), $custom_ot_dis = array() ) {    	
        $amount = 0;
    ...
    //new
       public static function computePrevSpecialAmount($attendance, $rate, $daily_amount, $hourly_amount, $custom_ot = array(), $custom_ot_dis = array() ) {    	
        $amount = 0;
      ...
      public static function computeCutSpecialAmount($attendance, $rate, $daily_amount, $hourly_amount, $custom_ot = array(), $custom_ot_dis = array() ) {    	
        $amount = 0;
    ...
    //new
    public static function computeSpecialAmountNoOmit($attendance, $rate, $daily_amount, $hourly_amount, $custom_ot = array(), $custom_ot_dis = array()) {    	
        $amount = 0;
    ...
    //new
        public static function computePrevRestDaySpecialAmount($attendance, $rate, $daily_amount, $hourly_amount, $custom_ot = array()) {
        $amount = 0;
   	 ...
   	 public static function computeCutRestDaySpecialAmount($attendance, $rate, $daily_amount, $hourly_amount, $custom_ot = array()) {
        $amount = 0;
    ...
    //newstart
    public static function computePrevRestDayLegalOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 25", strtotime($date));
    ...
    public static function computeCutRestDayLegalOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 01", strtotime($date));
    ...
    //new
    public static function computePrevSpecialOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
									$date_start = date("M 25", strtotime($date));
    ...
    public static function computeCutSpecialOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
									$date_start = date("M 01", strtotime($date));
    ...
    //new
    	public static function computePrevRestDaySpecialOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
										$date = date("M d", strtotime($a->getDate()));
										$date_start = date("M 25", strtotime($date));
    	...
    	public static function computeCutRestDaySpecialOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
										$date = date("M d", strtotime($a->getDate()));
										$date_start = date("M 01", strtotime($date));
    ...
    public static function computeRestDayLegalAmount($attendance, $rate, $daily_amount, $hourly_amount) {
   	...
   	//newstart
   	public static function computePrevRestDayLegalAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 25", strtotime($date));
	...
	public static function computeCutRestDayLegalAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 01", strtotime($date));
            ...
            if ($a->isPresent() && $a->isHoliday() && $a->isRestday()) {
                ...
                if ( !empty($h) && $h->isSpecial()) {
                    ...
                    if($t->getScheduledTimeIn() != '' && $t->getScheduledTimeIn() != '') {
	                   ...
	                   $ns_rate = $rate->getHolidaySpecialRestDayNightShiftOvertime();
	                    //$hourly_amount = $hourly_amount * $day_rate * $ot_rate;
    ...
    //newstart
        public static function computePrevRestDaySpecialOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        			$date = date("M d", strtotime($a->getDate()));
											$date_start = date("M 25", strtotime($date));
     ...
     public static function computeCutRestDaySpecialOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        			$date = date("M d", strtotime($a->getDate()));
											$date_start = date("M 01", strtotime($date));
        ...
        foreach ($attendance as $a) {
            if ($a->isPresent() && $a->isHoliday() && $a->isRestday()) {
                ...
                if ( !empty($h) && $h->isLegal()) {
                    ...
                    $ot_rate = $rate->getHolidayLegalRestdayOvertime() / 100;
                   //old $ns_rate = ($rate->getNightShiftDiff() - 100) / 100;
                   $ns_rate = $rate->getHolidayLegalRestDayNightShiftOvertime();
    ...
    //startnew
    public static function computePrevRestDayLegalOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        			$date = date("M d", strtotime($a->getDate()));
											$date_start = date("M 25", strtotime($date));
     ...
     public static function computeCutRestDayLegalOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
        			$date = date("M d", strtotime($a->getDate()));
											$date_start = date("M 01", strtotime($date));
    ...
    //new
        public static function computePrevRestDayAmount($attendance, $rate, $daily_amount, $hourly_amount, $custom_ot = array()) {
        $amount = 0;
        foreach ($attendance as $a) {
    ...
    public static function computeCutRestDayAmount($attendance, $rate, $daily_amount, $hourly_amount, $custom_ot = array()) {
        $amount = 0;
        foreach ($attendance as $a) {
    ...
    //new
     public static function computePrevRestDayOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
     ...
     public static function computeCutRestDayOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;
        foreach ($attendance as $a) {
    ...
    //new
    public static function computeCutRegularOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;  
        $hrly_amount = $hourly_amount;      
        foreach ($attendance as $a) {
        		$date = date("M d", strtotime($a->getDate()));
    ...
    public static function computePrevRegularOvertimeAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $amount = 0;  
        $hrly_amount = $hourly_amount;      
        foreach ($attendance as $a) {
        		$date = date("M d", strtotime($a->getDate()));
    ...
    */
    ...
    //newstart
    public static function computePrevRestDayOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 25", strtotime($date));
    ...
    public static function computeCutRestDayOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 01", strtotime($date));
    ...
    //new
 public static function computeCutRegularOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {    	
        $total = 0;
        $rate_per_hour   = $hourly_amount; 
        $nightshift_rate = ($rate->getNightShiftDiff() - 100) / 100;       
        foreach ($attendance as $a) {
     ...
     public static function computePrevRegularOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {    	
        $total = 0;
        $rate_per_hour   = $hourly_amount; 
        $nightshift_rate = ($rate->getNightShiftDiff() - 100) / 100;       
        foreach ($attendance as $a) {
    ...
    //new
 public static function computeCutRegularNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {    	
        $total = 0;
        foreach ($attendance as $a) {
        	 $date = date("M d", strtotime($a->getDate()));
                $date_start = date("M 01", strtotime($date));
 ...
 public static function computePrevRegularNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {    	
        $total = 0;
        foreach ($attendance as $a) {
        	 $date = date("M d", strtotime($a->getDate()));
                $date_start = date("M 25", strtotime($date));
    ...
    //newstart
    public static function computePrevRestDayNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 25", strtotime($date));
     ...
     public static function computeCutRestDayNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 01", strtotime($date));
    ...
    //newstart
    public static function computePrevRestDaySpecialNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
			$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 25", strtotime($date));
    ...
    public static function computeCutRestDaySpecialNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
			$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 01", strtotime($date));
    ...
    //newstart
    public static function computePrevRestDayLegalNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        				$date = date("M d", strtotime($a->getDate()));
												$date_start = date("M 25", strtotime($date));
     ...
     public static function computeCutRestDayLegalNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        				$date = date("M d", strtotime($a->getDate()));
												$date_start = date("M 01", strtotime($date));
    ...
    //newstart
    public static function computePrevSpecialNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 25", strtotime($date));
    ...
    public static function computeCutSpecialNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
			$date_start = date("M 01", strtotime($date));
    ...
    //new
public static function computeCutLegalNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        			 $date = date("M d", strtotime($a->getDate()));
                $date_start = date("M 01", strtotime($date));
    ...
    public static function computePrevLegalNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        			 $date = date("M d", strtotime($a->getDate()));
                $date_start = date("M 25", strtotime($date));
    ...
    //new
 public static function computeCutLegalOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        		 $date = date("M d", strtotime($a->getDate()));
                $date_start = date("M 01", strtotime($date));
    ...
    public static function computePrevLegalOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        		 $date = date("M d", strtotime($a->getDate()));
                $date_start = date("M 25", strtotime($date));
    ...
    //newstart
     public static function computePrevSpecialOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
									$date_start = date("M 25", strtotime($date));
    ...
    public static function computeCutSpecialOvertimeNightShiftAmount($attendance, $rate, $daily_amount, $hourly_amount) {
        $total = 0;
        foreach ($attendance as $a) {
        	$date = date("M d", strtotime($a->getDate()));
									$date_start = date("M 01", strtotime($date));
