...
class G_Company extends Company {
    ...
    public function generateWeeklyPayslip($month, $cutoff_number, $year = '', $excluded_employee_deduction = array()) {
       ...
       //monthly generatepayslip -alex

      public function generateMonthlyPayslip($month, $cutoff_number, $year = '', $excluded_employee_deduction = array()) {
        if ($year == '') {
            $year = Tools::getGmtDate('Y');
        }
        ...
        foreach($inactive_emp as $empkey => $empd) {
            $employee_id = $empd->getId();
            if($employee_id) {
                $e_payslip = G_Monthly_Payslip_Finder::findByEmployeeIdAndPeriod($employee_id, $from_date, $to_date);
                if($e_payslip) {
                    $e_payslip->delete();
                }
            }
        ...
        $period = G_Monthly_Cutoff_Period_Finder::findByPeriod($from_date, $to_date);  
        //$period = G_Cutoff_Period_Finder::findByYearMonthAndCutoffNumber($year, $month, $cutoff_number);        
        if (!$period) {
            return false;
        }
