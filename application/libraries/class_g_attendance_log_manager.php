...
class G_Attendance_Log_Manager {
    ...
    /*
     *  $multiple_attendance - array of G_Attendance_Log
     */
    public static function saveMultiple($multiple_attendance) {
        ...
        if ($has_record) {
            ...
            $sql_insert = "
                INSERT INTO ". G_ATTENDANCE_LOG ." (id, user_id, employee_code, date, time, type, remarks, employee_name)
	...
	/*
		$a - Instance of G_Attendance_Log
	*/
	public static function save($a) {
		...
		$sql = $sql_start ."
			...
			employee_code = '". $a->getEmployeeCode() ."',
			date = ". Model::safeSql($a->getDate()) .",
			time = ". Model::safeSql($time) .",
			type = ". Model::safeSql($type) .",
			employee_name = ". Model::safeSql($a->getEmployeeName()) .",
			remarks = ".Model::safeSql($a->getRemarks())."
			". $sql_end ."		
		";	
	...
	*/		
	...
	public static function updateUntransfferedDataByDate($date) {
		$sql = "
			UPDATE ". G_ATTENDANCE_LOG ."
			SET is_transferred =" . Model::safeSql(G_Attendance_Log::IS_TRANSFERRED) . "
	...
	public static function importLogs( $file = '' ) {		
		...
		$sql 		          = "INSERT IGNORE INTO " . G_ATTENDANCE_LOG . "(employee_code, type, date, time)VALUES";
