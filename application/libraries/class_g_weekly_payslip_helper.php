...
class G_Weekly_Payslip_Helper
{
	...
	public static function generatePayslipByEmployeeIdsPeriod($employee_ids, G_Weekly_Cutoff_Period $period)
	{
		$payslips = array();
	...
	public static function sqlGetPreviousEmployeePayslipDetailsByEmployeeId($employee_id = 0, $date_start = '', $date_end = '', $fields)
	{
		...
		if ($previous_cutoff['start_date'] != '' && $previous_cutoff['end_date'] != '') {
			...
			$sql = "
				SELECT {$sql_fields}
				FROM g_employee_weekly_payslip
	...
	public function sqlAllEmployeesSSSContributionByDateRangeNoDup($date_from = '', $date_to = '', $add_query)
	{
		...
		$getSSS = G_Settings_Weekly_Deduction_Breakdown_Finder::findByName('SSS');

		if($getSSS){
			$breakdownContainer = explode(':', $getSSS->getBreakdown());

			$c = G_Weekly_Cutoff_Period_Finder::findByPeriod($date_from, $date_to);
			$percentage = $breakdownContainer[$c->getCutoffNumber()-1]/100;
		}
		...
		FROM g_employee_weekly_payslip p
			...
			LEFT JOIN g_settings_employment_status es ON e.employment_status_id = es.id
			LEFT JOIN " . G_EMPLOYEE_SUBDIVISION_HISTORY . " esh ON e.id = esh.employee_id AND esh.end_date = ''
			LEFT JOIN p_sss sss ON floor(p.sss/".$percentage.") = floor(sss.employee_share+sss.provident_ee) 
    ...
    */
	...
	public static function updatePayslip(IEmployee $e, $start_date, $end_date)
	{
		...
		$period = G_Weekly_Cutoff_Period_Finder::findByPeriod($start_date, $end_date);
	...
	{
		$c = G_Weekly_Cutoff_Period_Finder::findByDate($date);
		if ($c) {
	...
	{
		$c = G_Weekly_Cutoff_Period_Finder::findByPeriod($start_date, $end_date);
		$employees = G_Employee_Finder::findAllActiveByDate($start_date);
		foreach ($employees as $id => $e) {
			$p = G_Weekly_Payslip_Helper::generatePayslip($e, $start_date, $end_date);
			if ($p) {
				if ($c) {
			...
			if ($a->isHoliday() && !$a->isOfficialBusiness()) {
				...
				if ($h && $h->isLegal()) {
					...
					if ($specific_sched || $sched) {
						...
						$cutoff_period = G_Weekly_Cutoff_Period_Helper::sqlGetCurrentCutoffPeriod($date);
