<?php
class G_Employee_Rest_Day_Request_Helper {
		
	public static function isIdExist(G_Employee_Rest_Day_Request $e) {
		$sql = "
			SELECT COUNT(*) as total
			FROM " . G_EMPLOYEE_REST_DAY_REQUEST ."
	...
	public static function findByDate($date,$order_by,$limit)
	{
		$sql = "
			SELECT a.*
			FROM ". G_EMPLOYEE_REST_DAY_REQUEST ." a
			WHERE
	...
	public static function findByCompanyStructureId($csid,$order_by,$limit)
	{
		$sql = "
			...
			a.is_approved,
			CONCAT(e.lastname, ' ', e.firstname) AS employee_name,e.hash, l.name as leave_type
			FROM ". G_EMPLOYEE_REST_DAY_REQUEST ." a, g_employee e, g_leave as l
			WHERE
				a.company_structure_id = ". Model::safeSql($csid) ."
	...
	public static function countTotalRecords($csid)
	{
		$sql = "
			...
			FROM ". G_EMPLOYEE_REST_DAY_REQUEST ." a, g_employee e
			WHERE
				a.company_structure_id >= ". Model::safeSql($csid) ."
