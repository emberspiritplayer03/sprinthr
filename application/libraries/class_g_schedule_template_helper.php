<?php
class G_Schedule_Template_Helper {
	
	/*
		returns array from G_Schedule_Template_Finder
	*/
	public static function getCurrentEmployeeSchedule(IEmployee $e) {
		$date = Tools::getGmtDate('Y-m-d');	
		$s = G_Schedule_Template_Finder::findActiveByEmployee($e, $date);
		if ($s) {
			$schedule_type_id = $s->getScheduleTypeId();
			$schedules = G_Schedule_Template_Finder::findAllByScheduleGroupId($schedule_type_id);
		} else {
			$schedules = G_Schedule_Template_Finder::findAllDefault();
		}
		return $schedules;
	}
	
	/*
		$schedules - Returned value from G_Schedule_Template_Finder
	*/
	...
	public static function showSchedulesWithHours($schedules) {
		$string = '<div class="styled_items_holder"><ul>';
		foreach ($schedules as $schedule) {
			$string .= '<li><div class="item-detail-styled"><i class="icon-time icon-fade vertical-middle"></i> ';
			$string .= '<strong>'. $schedule->getWorkingDays() . '</strong>';
			$string .= ' ('. $schedule->getHours().' hours)';	
			$string .= '</div></li>';
		}
		$string .= '</div></ul>';
	...
	public static function isGroupAlreadyAssigned(IGroup $g, G_Schedule_Template $s) {
		define('ENTITY_GROUP', 2);
		$sql = "
			SELECT COUNT(*) as total
			FROM v2_employee_schedule_type s
			WHERE s.employee_id = ". Model::safeSql($g->getId()) ."
	...
	public static function isEmployeeAlreadyAssigned(IEmployee $e, G_Schedule_Template $s) {
	...
	public static function isEmployeeAlreadyAssignedNonMultipleSched(IEmployee $e, G_Schedule_Template $s) {
		define('ENTITY_EMPLOYEE', 1);
		$sql = "
			SELECT COUNT(*) as total
			FROM v2_employee_schedule_type s
	...
	public static function countMembers(G_Schedule_Template $s) {
		$sql = "
			SELECT COUNT(*) as total
			FROM v2_employee_schedule_type s
	...
	public static function mergeByName(G_Schedule_Template $schedule) {			
		$schedules = array();
	...
	public static function getHours($day, $schedules) {
		foreach ($schedules as $s) {
			$working_days = $s->getWorkingDays();
			$days = explode(',', $working_days);
			if (in_array($day, $days)) {
				$return['hours'] = $s->getHours();
				return $return;
			}			
		}
		return false;
	...
	public static function loadArrayEmployeeSchedule($eArray) {		
		if(!empty($eArray)){
			...
			foreach($eArray as $eid){				
				...
				if($e){
					$ss = G_Employee_Schedule_Type_Helper::getAllScheduleByEmployeeGroupId($e->getId());
					if($ss){
						$e_array[$e->getId()] = $ss;
					}										
				}else{
