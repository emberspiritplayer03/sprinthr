...
class G_Overtime_Helper {
	...
	public static function isOvertimeExist($employee, $auto_file_ot_date_in) {
		$employee = Model::safeSql($employee);
		$auto_file_ot_date_in = Model::safeSql($auto_file_ot_date_in);
		$sql = "
			SELECT COUNT(*) as total
			FROM " . G_EMPLOYEE_OVERTIME ."
			WHERE employee_id = ". $employee ." AND date = ". $auto_file_ot_date_in ."
		";
		// echo $sql;
		$result = Model::runSql($sql);
		$row    = Model::fetchAssoc($result);
	...
	*/	
	...
	public static function getAllByUnRequest() {
		$sql = "
			...
			FROM " . G_EMPLOYEE_OVERTIME ." 
			WHERE id NOT IN ( SELECT request_id FROM ". REQUESTS ." WHERE request_type = 'ot') AND is_archived = 'No'
		";		
