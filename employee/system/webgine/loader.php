...
class Loader
{
	...
	{
		$path = $_SERVER['DOCUMENT_ROOT'] . BASE_FOLDER .'application/libraries' . LIBRARY_FOLDER;
		if (is_array($class))
		{
	 ...
	 * @param mixed $class
	 */
	public static function sysMainLibrary($class) {
		$path = $_SERVER['DOCUMENT_ROOT'] . MAIN_FOLDER .'/'. SYS_FOLDER .'/libraries/';
		if (is_array($class)) {
			foreach ($class as $c) {
				$class = strtolower($c);
				if (!file_exists($path . $c . EXT)) {
	...
	public static function appScript($file) {
		...
		//echo self::$script_file[] = BASE_FOLDER . APP_FOLDER . '/scripts/' . $file;
		self::$script_file[] = BASE_FOLDER. 'application/scripts/' . $file;
	}
	
	public static function appMainScript($file) {
		//self::$script_file[] = BASE_FOLDER . APP_FOLDER . '/scripts/' . $file;
		//echo self::$script_file[] = BASE_FOLDER . APP_FOLDER . '/scripts/' . $file;
		self::$script_file[] = MAIN_FOLDER . 'application/scripts/' . $file;
	}	
	...
	public static function appMainStyle($file) {
		self::$scriptstyle_to_load .= '<link rel="stylesheet" href="'. MAIN_FOLDER . 'themes/' . MAIN_THEME . '/' . $file . '" />|';
	}	
	...
	public static function appMainUtilities() {
		
		Loader::appMainScript('generic.js');
		Loader::appMainStyle('assets/generic.css');
		Loader::sysMainLibrary('class_tool');
		Loader::sysMainLibrary('class_validate');
	}
	...
	*/
	...
	public static function includeMainScript($file) {
		self::$script_to_include[] = MAIN_FOLDER . $application_folder . "/scripts/" . $file;
